...
\textbf{Results}: Some of the results of this work was a mean correlation of 0.4 between \textit{WIP} and \textit{throughput}, a mean correlation of 0.2 between \textit{WIP} and both \textit{bugs} and \textit{lead time} and a mean correlation of -0.1 between \textit{WIP} and  \textit{churn} across the teams.\\ 
\textbf{Conclusion}:
...
\label{sub:sec:bfq} 
To get the statistics on number of bugs finished the same quarter as it was recorded, the software program and SPSS were used. The program extracts the \textit{created date} and the \textit{date to} values from each task and checks if their quarter and year match. If they do, a boolean value represented in the task is set to true, otherwise it is set to false. The output produced by the software program is used by SPSS, where the boolean value will be grouped by \textit{team name, type} and \textit{quarter}.  
