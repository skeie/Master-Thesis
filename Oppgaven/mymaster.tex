...
\textbf{Aim}: 
The aim of this work is to investigate the impact that WIP-limits have on software development. \\
\textbf{Methods}:
The methods used to investigate the research question were a case study of an in-house software development company. The case study was based on a data set with meta-data about each of the tasks that the software company worked on from 2008 to 2013. The data set was analyzed using an application developed for, and later described in this work. From the data set, the application measured variables such as \textit{WIP, throughput, bugs, lead time} and \textit{churn} for each team. The data produced by the application was interpreted with correlations and case summaries in statistical application. Correlation is a statistical method that measures how two variables change in relation to each other.  Case summaries is a statistical method for grouping variables and calculate descriptive statistics. The correlation between variables is used to investigate the impact of WIP-limits.\\
\textbf{Results}: Some of the results of this work were a mean correlation of 0.4 between \textit{WIP} and \textit{throughput}, a mean correlation of 0.2 between \textit{WIP} and both \textit{bugs} and \textit{lead time} and a mean correlation of -0.1 between \textit{WIP} and  \textit{churn} across the teams.\\ 
\textbf{Conclusion}:
...
This is the reason why this work will focus on software development methods. The methods in each development project are such a key element to make a project successful. The main focus of this work will be the Kanban method and the principle WIP-limit. In Kanban is the WIP-limit used to  limit the number of tasks each developer can work on at each workflow state, to prevent bottlenecks and to ensure flow of tasks through the development cycle \parencite{gandomani2013important} \parencite{ikonen2010exploring}.
...
Chapter \ref{ch:res} presents the result produced by the developed software application and SPSS. The result is presented with descriptive statistics and correlation tables. 
...
\section {Scrum}
\label{sec:Scrum}
"Scrum is the best-known of the Agile frameworks. It is the source of much of the thinking behind the values and principles of the Agile Manifesto"\parencite{Scrum}. The values of the Agile Manifesto are \parencite{Scrum}:
...
Figure \ref{table:teams} shows the size of the ten teams vs. quarter. The team size is used as a variable to compute the result presented in Chapter \ref{ch:res}. Team seven, shown in Table \ref{Team:7} contributes data from 2010 to 2012. After 2012, team seven was shut down.
...
\item All seven days in the week are considered when the software application calculates.
...
\label{sec:workflow}
The workflow for creating the result in Chapter \ref{ch:res} is showed in Figure \ref{dataflow}. First is the data converted from Microsoft TFS server to an excel document. The excel document is then converted to a .csv file. The .csv file is measured by the developed software application. The application produce documents containing WIP, throughput, lead time etc based on the .csv file. Then the documents produced by the application are used by SPSS to compute correlation tables and descriptive statistics.  
\begin{figure}[h] 
  \center
...
\vspace{-2.0em}
The each teams' Arraylists from section \ref{sub:stepOne}  and \ref{sub:stepTwo} now contains a WIP object for each date. In this step, the program will loop through each of the teams Arraylists. During the iteration each WIP object is extracted from the Arraylist and the WIP is measured. The two methods stated in line 10 and 17 in Listing \ref{lst:measure} respectively gather how many tasks there were in process on that date (method in line 10) and finds how many tasks are finished on that date (method in line 18) and returns the result. The result is used in line 6 to compute the current WIP. The conditional statement on line 4 assures that only one instance of each date is measured. 
\begin{minipage}{\textwidth} 
...
\label{sub:sub:st3}
The Arraylist now contains the dates from 2010-10-07 to 2010-10-13. The last step is to measure WIP for each date.  The program will now loop through the Arraylist. The first date in the arraylist is 2010-10-07.  The get\_current\_wip method from line 9 in Listing \ref{lst:measure} will be called with the date 2010-10-07 as parameter.  The method will return two, because both tasks one and two were started at 2010-10-07 as shown by Figure \ref{wip_timeline}. The next thing to do is to find out how many tasks to decrement the current WIP with.  The method Nr\_of\_finished\_dates in line 17 is called with the date 2010-10-07. The Figure \ref{wip_timeline} shows that there was no task finished at the date 2010-10-07, so the method returns 0. The program then updates the WIP objects' counter to be two and saves the WIP  value in the lastWIP variable. The next date is 2010-10-08, which the program made in Subsection \ref{sub:sub:st}. There is no task started at 2010-10-08, but task 1 is finished at the date. So the Nr\_of\_finished\_dates returns one and flags the current date as shown in Listing \ref{lst:measure} by the line 23. The result of WIP\_measure in line 5 is 1, because the WIP from previous date was 2 ($0-1+2=1$). Therefore the WIP at  2010-10-08 1, as shown by Figure \ref{wip_timeline}. The program will continue this procedure until all the dates are measured.  The reason why the date is flagged in Nr\_of\_finished\_dates method is to be sure that each date is only evaluated ones.
\iffalse
...
\label{sec:bug}
To measure the moderating variables for \textit{churn} and \textit{throughput}, the developed software application and SPSS were used. The application will generate \textit{throughput} and \textit{churn} as described in Sections \ref{sec:TP} and \ref{sec:churn}, the output from the software application will look like Table \ref{tab:ftb}. The output from the program will be used by SPSS. SPSS will use a function called case summaries. The case summaries function groups variables based on a common value.  With the case summaries function the variables \textit{team name}, \textit{quarter} and \textit{type} will group the variables \textit{churn} and \textit{throughput}. The result from case summaries provides the moderating variables for \textit{churn} and \textit{throughput} for each quarter.
\begin{table}[!ht]
\center
...
\label{sub:sec:bfq} 
To get the statistics on number of bugs finished the same quarter as it was recorded, the software application and SPSS were used. The program extracts the \textit{created date} and the \textit{date to} values from each task and checks if their quarter and year match. If they do, a boolean value represented in the task is set to true, otherwise it is set to false. The output produced by the software application is used by SPSS, where the boolean value will be grouped by \textit{team name, type} and \textit{quarter}.  
\subsection{Average days backlog, bug}
\label{sub:sec:adbb}
To get the statistics on the average number of days bugs are in backlog per quarter, the software application measures the number of days between the \textit{created date} and the \textit{date from} value. The result is saved together with the task. SPSS is used on the output of the program to measure the average days for bugs per quarter.
...
\chapter{Results}                     
\label{ch:res}
The first result was conducted without considering team size. This result showed a mean correlation  of 0.6 between \textit{team size} and \textit{WIP}, a mean correlation of 0.5 between \textit{team size} and \textit{throughput}, a mean correlation of 0.4 between \textit{bugs} and \textit{team size} and a mean correlation of 0.3 between \textit{lead time} and \textit{team size}. Based on these mean values, it was hard to find any evidence that WIP-limit matters in software development. This resulted in a new analyze where each variable for each quarter was divided by the corresponding team size.

The correlation and descriptive statistic tables for \textit{churn}, \textit{lead time}, \textit{throughput} and \textit{bugs} are represented in this chapter, so the mean correlation between these variables can be investigated  with respect of WIP-limits in Chapter \ref{ch:dis}.

Each section except Sections \ref{sec:corr:tz}, Section \ref{sec:WIP:limit} and Section \ref{sec:TP:team}  are presented with two correlation tables and two corresponding descriptive statistics tables. One correlation table and descriptive statistic table for the first analysis and the same two tables for the second analysis. Section \ref{sec:corr:tz} is presented with one correlation table and one descriptive statistic table. Sections  Section \ref{sec:WIP:limit} and Section \ref{sec:TP:team} are presented  with one WIP and throughput figure for each team. The content of the sections will consist of highlighting the variables with a significant correlation, describe the descriptive statistic tables and describe the team figures.
...
In Table \ref{corr:WIP:V2}, \textbf{teams two} and \textbf{three} have a significant correlation  for two of the three \textit{throughput} variables. \textbf{Team four} has a positive correlation for two of the three \textit{churn} and \textit{throughput} variables and \textbf{team eight} has a significant correlation to one of the three \textit{churn} variables. The relationship between these moderating variables is explained in Section \ref{sec:diss:sv}. The descriptive statistic Table \ref{DS:corr:WIP:v2} shows \textit{throughput} has a mean correlation of 0.4 between \textit{WIP}. The rest of the variables except \textit{throughputs} moderating variables have a mean correlation of $\pm$ 0.2 or less. 
...
Table \ref{corr:lt} displays the variances between the \textit{throughput} variables for \textbf{teams one, two, four} and \textbf{eight}. There is also variance for the \textit{churn} variables for \textbf{teams one, three, four} and \textbf{eight}. The relationship between these variables is explained in Section \ref{sec:diss:sv}. Table \ref{DS:corr:LT} show the mean correlation between \textit{lead time} and both \textit{WIP} and \textit{throughput} are 0.5. \textit{Throughput feature} has the mean correlation of 0.4 between \textit{lead time} and \textit{throughput bug, bugs, churn feature} and \textit{team size} have the mean correlation of 0.3 between \textit{lead time}. The rest of the values have a correlation of $\pm$ 0.2 or less between \textit{lead time}.
...
\label{sec:corr:TP}
This section shows the correlation table between \textit{throughput} and the variables. The first correlation Table \ref{corr:TP} shows the correlation when team size is \textbf{not} considered. In the table one can see that \textit{throughput} has a significant correlation to either \textit{throughput feature} or \textit{throughput bug} for each of the teams. The \textbf{teams three, four, five, seven} and \textbf{nine} have a positive correlation to both the \textit{throughput} moderating variables. For \textbf{teams one, six} and \textbf{eight} are the variance between \textit{throughput feature} and \textit{throughput}.  For \textbf{teams two} and \textbf{ten} are the variance between \textit{throughput bug} and \textbf{throughput}, according to Table \ref{corr:TP}. The relationship between the \textit{throughput} variables that have a variance is explained in Section \ref{sec:diss:sv}.
...
For \textbf{teams one, four, six} and \textbf{eight} are there a variance between the \textit{churn} variables, the relationship between these variables is explained in Section \ref{sec:diss:sv}.
...
The second correlation table, Table \ref{corr:TP:V2} shows the correlation between \textit{throughput} and the variables when team size is considered. In the table one can see \textit{throughput} has a significant correlation to either \textit{throughput feature} or \textit{throughput bug} for each of the teams. The \textbf{teams two, three} and \textbf{nine} have a positive correlation to both the \textit{throughput} moderating variables. For \textbf{teams five, eight} and \textbf{ten} there are a variance between \textit{throughput feature} and \textit{throughput}.  For \textbf{teams one, four, six} and \textbf{seven} are there a variance between \textit{throughput bug} and \textit{throughput}, according to Table \ref{corr:TP:V2}. The relationship between the \textit{throughput} variables that have a variance is explained in Section \ref{sec:diss:sv}.
...
The \textit{churn} variables for \textbf{teams four, five} and \textbf{eight} have a variance, according to Table \ref{corr:TP:V2}. The \textit{churn} relationship is explained in Section \ref{sec:diss:sv}. 
...
\textbf{Teams one, five} and \textbf{six} have a variance for both \textit{throughput} and \textit{churn} variables, according to Table \ref{corr:Teams}. \textbf{Teams seven, eight} and \textbf{ten} have a variance between the \textit{throughput} variables, while \textbf{team three} has it between the \textit{churn} variables. The relationship between the moderating variables is explained in Section \ref{sec:diss:sv}. The Table \ref{DS:corr:TS} display a mean correlation of 0.6 for  \textit{WIP} between \textit{team size}, 0.5 for \textit{throughput} between \textit{team size}, 0.4 between \textit{team size} and the \textit{throughput} moderating variables and \textit{bugs}. \textit{Lead time} has the mean correlation of  0.3 between \textit{team size}. Rest of the variables has a correlation of $\pm$ 0.2 or less between \textit{team size}.
...
\label{sec:WIP:limit}
\vspace{-1em}
To investigate if WIP-limit matter, the WIP-limit for each of the teams were measured.  \textit{Team one} has a mean WIP-limit of 0.73. \textit{Team two} has a mean WIP-limit of 2.31. \textit{Team three} has a mean WIP-limit of 1.78. \textit{Team four} has a mean WIP-limit of 3.21. \textit{Team five} has a mean WIP-limit of 1.93. \textit{Team six} has a mean WIP-limit of 3.02. \textit{Team seven} has a mean WIP-limit of 2.03 .\textit{Team eight} has a mean WIP-limit of 0.71. \textit{Team nine} has a mean WIP-limit of 2.19 and \textit{team ten} has a mean WIP-limit of 1.08, as showed in Figure \ref{table:WIP:teams}. This gives a mean WIP-limit of 1.9 per developer. 
...
The last Section \ref{sec:diss:sv} will consist of an explanation of the relationship between the moderating variables. To back up any assumptions about the relationship between the moderating variables, correlation graphs and descriptive statistic tables listed in Appendix \ref{app:DS} will be used. 

\section{WIP and Team size}
The results in Chapter \ref{ch:res} show the importance of taken team size into account when a case study like this is conducted. The results showed that the mean correlation between  \textit{WIP}  and \textit{throughput} were 0.6 and the mean correlation between \textit{WIP} and both  \textit{bugs}  and \textit{lead time} were  0.5. These three mean correlations are decreased by 0.2  and 0.3 when team size was considered.  The results also showed the mean correlation between \textit{WIP} and \textit{team size} were 0.6, the mean correlation between \textit{team size} and \textit{throughput} were 0.5, the mean correlation between \textit{team size} and \textit{bugs} were  0.4 and the mean correlation between  \textit{lead time} and \textit{team size} were 0.3.  Based on these variables, the results from when team size was \textbf{not} considered are discarded. Because the data showed that \textit{team size} had a great impact on the correlation tables and possible gave biased results.
...
The mean correlation between \textit{lead time} and \textit{throughput} are 0.5, as showed by Table \ref{DS:corr:TP:v2}. A positive correlation between these variables states that when \textit{throughput} increases, the time from when a task is taken from the backlog until it is finished also increases and vice versa.
This correlation relationship should be negative. Because if there are produced more tasks, the total time to produce tasks should be minimized.

There are five teams with a positive significant correlation, as showed by \ref{corr:TP:V2}. The mean WIP-limits for these five teams are 2.31, 3.21, 1.93, 3.02 and 1.08, as showed by Figure \ref{table:WIP:teams}. These WIP-limits and correlations show no pattern that point towards the fact that WIP-limits matter or that they are too high or low. Because the teams with a significant correlation have both high and low WIP-limits.

The four teams with the lowest WIP-limits have the correlation of 0.06, 0.26, 0.24 and 0.90, showed by \ref{corr:WIP:V2} and Figure \ref{table:WIP:teams}. Three of the five teams with the significant correlations are also the three teams with the highest mean WIP-limits. Three of the four with the lowest WIP-limits also have the lowest correlation between \textit{lead time} and \textit{throughput}. This data shows that most of the teams with highest mean WIP-limits have a significant correlation. And the teams with the lowest mean WIP-limits are the one closes to have a negative correlation between \textit{throughput} and \textit{lead time}. Although the data show evidence that WIP-limits reduce the correlation between \textit{lead time} and \textit{throughput}, there is no evidence that lowering WIP-limits even more will result in a negative correlation between \textit{lead time} and \textit{throughput}.  But, based on these data, it looks like WIP-limits matter, because it decrease the correlation between \textit{lead time} and \textit{throughput}. Based on this as well, it looks like the WIP-limits may have been set too high. 

The mean correlation between \textit{bugs} and \textit{throughput} are 0.6, seen in Table \ref{DS:corr:WIP:v2}. A positive correlation between \textit{bugs} and \textit{throughput} states that if SI produces more, then they produce more bugs and vice versa. This correlation relationship is standard. When one produces more tasks, there are likely to produce more bugs. If one produces zero tasks, one also produce zero bugs. In an optimal process, the correlation between \textit{bugs} and \textit{throughput} should be a negative correlation, so teams can produce more without producing bugs. This is the principle of \textit{JIT}, explained in Section \ref{sec:JIT}.


There are six teams with a significant positive correlation between \textit{throughput} and \textit{bugs} as showed by Table \ref{corr:WIP:V2}. These six teams have a mean WIP-limit of 2.31, 1.78, 1.93, 0.71, 2.19 and 1.08, as showed by Figure \ref{table:WIP:teams}. These values indicates no pattern between WIP-limits  and the correlations between \textit{bugs} and \textit{throughput}. \textbf{Team one} and \textbf{six} are the only teams with a correlation lower than 0.45. \textbf{Team one} has a correlation of 0.05 and a WIP-limit of 0.73. \textbf{Team six} has a correlation of -0.02 and a mean WIP-limit of 3.02, as showed by Table \ref{corr:WIP:V2} and Figure \ref{table:WIP:teams}.  These data strengthens the assumption that there is no pattern between WIP-limits  and the correlation between \textit{throughput} and \textit{bugs}, because teams with both high and low mean WIP-limits have significant correlation between \textit{throughput} and \textit{bugs}.  

The mean correlation between \textit{throughput} and \textit{bugs finished, quarter} are 0.4. The mean correlation between \textit{throughput} and \textit{throughput bug} are 0.8 and the mean correlation between \textit{throughput} and \textit{throughput feature} are 0.6, as showed by Table \ref{DS:corr:TP:v2}. The positive correlation between  \textit{throughput} and \textit{bugs finished, quarter} states that when SI produces more, they are able to fix more bugs within the same quarter, which shows the same as the correlation between \textit{bugs} and \textit{throughput}. The positive correlation between \textit{throughput} and both the moderating variables  state that SI produces more bugs than feature. These correlations also show the same as the correlation between \textit{throughput} and \textit{bugs}. 


The mean correlation between \textit{throughput} and \textit{churn} are 0.3, as showed by Table \ref{DS:corr:WIP:v2}. A positive correlation states that if the tasks require more effort, they are able to produce more and vice versa. Based on Kanban, one should have small tasks, as stated in Section \ref{sec:Kan}. So the correlation between \textit{throughput} and \textit{churn} should be negative based on the principle in Kanban.  There are three teams with a significant correlation, as showed by Table \ref{corr:WIP:V2}. These three teams have the mean WIP-limit of 3.21, 1.93 and 0.71, showed by Figure \ref{table:WIP:teams}. There are three teams with a negative correlation between \textit{throughput} and \textit{churn}. These negative correlations are -0.14, -0.14 and -0.28, as showed by Table \ref{corr:WIP:V2}. The teams with these negative correlations have the mean WIP-limits of 0.73, 3.02 and 2.19, as showed by Figure \ref{table:WIP:teams}. Both teams with a high mean WIP-limit and low WIP-limit have both a significant positive correlation and negative correlation.  This states that there is no pattern between WIP-limits and the correlation relationship between \textit{throughput} and \textit{churn}. 

The data in this section showed that WIP-limits had an impact on the correlation between \textit{throughput} and \textit{lead time}. The correlation between \textit{throughput} and \textit{lead time} also showed evidence that the WIP-limits could be too high. The other correlations presented showed no evidence of WIP-limits impact. Based on this should SI possibly lower their WIP-limits.

\section{WIP and lead time}
 \label{sec:dis:lt}
\vspace{-1em}
As stated in Section \ref{sec:in:lt}, lead time could be used to track how quickly software is delivered to customers. Each development process would like to get their lead time as low as possible. Section \ref{WIPsec} states that WIP-limits are important to reduce lead times. The correlation between \textit{lead time} and \textit{WIP} shows a mean correlation of 0.2, shown by Table \ref{DS:corr:WIP:v2}. A positive correlation between \textit{lead time} and \textit{WIP} states that \textit{lead time} increases when \textit{WIP} increases and vice versa. Based on former research, the correlation between \textit{WIP} and \textit{lead time} should be positive. 

%The rest of this section will contain a discussion about whether there is any evidence showing the impact of WIP-limits on the \textit{lead time} correlation. The correlation of $\pm$ 0.2, showed by Table \ref{DS:corr:LT:v2} will be discussed. The mean correlations from 0.2 to -0.2 are discarded, because they are too low.

The four teams with the highest correlation are \textbf{team two, four, six} and \textbf{seven}. They have the correlations of 0.67, 0.87, 0.32 and 0.77 between \textit{WIP} and \textit{lead time}. The correlations 0.67, 0.87 and 0.77 are significant correlations. \textbf{Team two, four, six} and \textbf{seven} have the mean WIP-limits of 2.31, 3.21, 3.02 and 2.03  The four teams with the lowest correlation are \textbf{teams one, three, eight, nine} and \textbf{ten}. They have the correlations of -0.09, -0.03, -0.09, -0.18, -0.05 and  the mean WIP-limits of 0.73, 1.78, 0.71, 1.08 and 2.19, showed by Table \ref{corr:WIP:V2} and Figure \ref{table:WIP:teams}.  As  stated, the correlation between \textit{WIP} and \textit{lead time} should be positive, so when WIP is decreased, the \textit{lead time} decrease as well. These data points towards the fact that WIP-limit may be too low, because four of the five teams with a WIP-limits of 2 or higher have a positive correlation of 0.32 or higher. The team with the highest WIP-limit also has the highest WIP, but the team with the second highest WIP-limit has the correlation of 0.32. The two other teams with the significant correlation between \textit{WIP} and \textit{lead time} have the third and five highest WIP-limits. This shows that a WIP-limit of 2 or higher for four of the five teams gives at least a moderate correlation. This shows that WIP-limits matters based on the correlation between \textit{WIP} and \textit{lead time} and that the WIP-limits may have been set too low.

The mean correlation between \textit{lead time} and \textit{bugs} are 0.5, as showed by Table \ref{DS:corr:LT:v2}.  The correlation means that if SI produces fewer \textit{bugs}, the \textit{lead time} for each task will decrease and vice versa. One should try to have a positive correlation.   Five teams have a positive significant correlation and these teams have a WIP-limit of 0.73, 2.31, 1.93, 2.03 and 1.08, showed by Table \ref{corr:LT:V2} and Figure \ref{table:WIP:teams}. There is no pattern between these variables, because the teams {WIP-limit} are both high and low. The two teams with a correlation lower than 0.44 between \textit{bugs} and \textit{lead time} have the WIP-limits of 3.02 and 0.71, showed by Table \ref{corr:LT:V2} and Figure \ref{table:WIP:teams}. The WIP-limits 3.02 and 0.71 are almost the highest and the lowest WIP-limits for SI. This data shows that teams with high and low WIP-limits have both high and low correlation between \textit{lead time} and \textit{bugs}. Based on these data the WIP-limits do not make an impact on the correlation between \textit{bugs} and \textit{lead time}, because there  is no pattern between the teams with high WIP-limit or low WIP-limit and the correlation between \textit{lead time} and \textit{bugs}.

%The \textbf{teams three, four} and \textbf{nine} have a correlation value of 0.44, 0.48, 0.54 and mean WIP-limits of 1.78, 3.21 and 2.19.

The mean correlation between \textit{lead time} and \textit{churn} are 0.4, showed by Table \ref{DS:corr:LT:v2}. The teams should have a positive correlation between  \textit{lead time} and \textit{churn}, so the developers can work on small tasks and delivery quickly, which is stated in Section \ref{sec:Kan}.  \textbf{Teams one, four, five} and \textbf{eight} have a significant positive correlations and these teams have mean WIP-limits of 0.73, 3.21, 1.93 and 0.71, showed by Table \ref{corr:LT:V2} and Figure \ref{table:WIP:teams}. These values show no pattern, there are both teams with high and low WIP-limits with significant correlation. The three teams with the lowest correlations are the \textbf{teams six, nine} and \textbf{ten}. These teams have the WIP-limits of 3.02, 2.19 and 1.08, showed by Table \ref{corr:LT:V2} and Figure \ref{table:WIP:teams}. Based on the WIP-limits and the correlation, WIP-limits cannot be used to regulate the relationship between  \textit{lead time} and \textit{churn}, because there is no pattern between WIP-limits and the correlation.

The mean correlation between \textit{lead time} and  \textit{bugs finished quarter} and are 0.5, as showed by Table \ref{DS:corr:LT:v2}. This relationship means that if \textit{lead time} decreases, the number of bugs closed in each quarter decreases and vice versa. The relationship between these variables should be negative, so one can close more bugs and minimize \textit{lead time}. \textbf{Teams one, two, five, seven} and \textbf{ten} have a significant positive correlation and these teams have the mean WIP-limits of 0.73, 2.31, 1.93, 2.03 and 1.08, showed by Table \ref{corr:LT:V2} and Figure \ref{table:WIP:teams}. There is one team with a negative correlation, which is \textbf{team eight}. \textbf{Team eight} has the correlation of -0.04 and the mean WIP-limit of 0.71. However, the two other teams with the lowest correlations have the correlations of 0.32 and 0.36. These two teams have the WIP-limits of 1.78 and 3.02, showed by Table \ref{corr:LT:V2} and Figure \ref{table:WIP:teams}. These data points towards the fact that WIP-limits have no impact on the correlation between \textit{lead time} and \textit{bugs finished quarter}, because there is no pattern between high or low WIP-limits and the correlations.

The former research states that WIP-limits have an impact on \textit{lead time}, the result from this work shows the same. The correlation between \textit{lead time} and \textit{WIP} showed evidence that WIP-limit matter. These data also showed evidence that the WIP-limit may have been set too low. The other correlations with a mean higher than $\pm$ 0.2 were investigated and no one of them showed a pattern towards the fact that WIP-limit helps \textit{lead time}.
...
\label{sec:dis:bugs}
The paper cited by Shinkle stated  \parencite{Shinkle}; when WIP-limits were too high, the number of bugs increased. The mean correlation between \textit{WIP} and \textit{bugs} are 0.2, showed by Table \ref{DS:corr:Bugs:v2}. A significant positive correlation states that if WIP decreases, so does bugs and vice versa. A positive correlation close to 1 between \textit{WIP} and \textit{bugs} should be the optimal goal.  There is one team with a significant correlation and that is \textbf{team seven}  with a correlation of 0.75 as showed in Table \ref{corr:WIP:V2}. \textbf{Team seven} has  a mean WIP-limit of  2.03 shown by Figure \ref{Team:WIP:7}. The second highest correlation is 0.49 and belongs to \textbf{team two}. \textbf{Team two} has the WIP-limit of 2.31, showed by Table \ref{corr:WIP:V2} and Figure \ref{Team:WIP:2}.  The teams with the lowest correlation between \textit{WIP} and \textit{bugs} are \textbf{team nine} and \textbf{team ten}. \textbf{Team nine} has the correlation of -0.05  and a mean WIP-limit of 2.19 and \textbf{team ten} has the correlation of -0.28 and a mean WIP-limit of 1.08, showed by Table \ref{corr:WIP:V2} and Figures \ref{Team:WIP:9} and \ref{Team:WIP:10}. The team with the highest WIP-limit, \textbf{team four} has the correlation of 0.27 between \textit{WIP} and \textit{bugs}.  The team with the lowest  WIP-limit, \textbf{team eight} has a correlation of 0.32, showed by Table \ref{corr:Bugs:v2} and Figure \ref{table:WIP:teams}.

These values show that the two teams with the highest correlations have the WIP-limits of 2.03 and 0.49.  The teams with the lowest correlation have the WIP-limits of 2.19 and 1.08. The team with the highest WIP-limit has the correlation of 0.27. The team with the lowest WIP-limit has the correlation of 0.32. Based on these values, there is no pattern between WIP-limits  and the correlation between \textit{WIP} and  \textit{bugs}.

The mean correlation between \textit{bugs} and \textit{bugs finished, quarter} are 0.5, showed by Table \ref{DS:corr:Bugs:v2}. This correlation states if SI produce more bugs, they are also able to close more bugs within each quarter and vice versa.
The correlation between \textit{bugs} and \textit{bugs finished, quarter} should be positive. \textbf{Team one, six, seven, eight} and \textbf{ten} have a significant positive correlation between \textit{bugs} and \textit{bugs finished, quarter}. These teams have a mean WIP-limit of 0.73, 3.02, 2.03, 0.71 and 1.08, as shown by Table \ref{corr:Bugs:v2} and Figure \ref{table:WIP:teams}. These values show no pattern between WIP-limit  and the correlation between \textit{bugs} and \textit{bugs finished, quarter}. There are two teams with a correlation lower than 0.20 and these teams have the WIP-limit of 3.21 and 2.19. The three remaining teams have the correlation of 0.44, 0.50 and 0.52 and  the WIP-limits of 2.31, 1.78 and 1.93, as showed by Table \ref{corr:Bugs:v2} and Figures \ref{Team:WIP:2}, \ref{Team:WIP:3} and \ref{Team:WIP:5}.  The WIP-limit for the teams with the correlation of 0.44, 0.50 and 0.52 shows some sort of a pattern, these WIP-limits are close.
The scattered WIP-limits for the significant correlation between \textit{bugs} and \textit{bugs finished, quarter} shows evidence towards the fact that the WIP-limits do not matter for the correlation relationship between \textit{bugs} and \textit{bugs finished, quarter}. It is not much to state with the two teams with the lowest correlation, since there are only two.
But, the conclusion is that it looks like the WIP-limits do not matter in the correlation relationship between  \textit{bugs} and \textit{bugs finished, quarter}, because the teams with significant correlations have a high scattering between the mean WIP-limits.

The \textit{avg days in backlog, bugs} has a mean correlation of 0.4 between \textit{bugs}, showed by Table \ref{DS:corr:Bugs:v2}. This means that the more bugs produced, the longer the bugs stays in the backlog and vice versa. The optimal correlation should be a positive one, so they can produce fewer bugs, and the average days bugs are in backlog decreases. There is one team with a negative correlation. That team is \textbf{team two} with a correlation of -0.07 between \textit{avg days in backlog, bugs} and \textit{bugs} and a WIP-limit of 2.31, showed by Table \ref{corr:Bugs:v2} and Figure \ref{Team:WIP:2}. There are two teams with a significant correlation, these teams are \textbf{team one} and \textbf{team five}. These two teams have the correlation of 0.76 and 0.75 and WIP-limits of 0.73 and 1.93, showed by Table \ref{corr:Bugs:v2} and Figures \ref{Team:WIP:1} and \ref{Team:WIP:5}. One cannot conclude anything with these three values, one need to take more values into account. There are six teams with close correlations. These correlations are 0.38, 0.30, 0.37, 0.38, 0.37 and 0.39. These teams have the WIP-limits of 1.78, 3.02, 2.03, 0.71, 2.19 and 1.08. These values show no pattern between WIP-limit  and the correlation between \textit{avg days in backlog, bugs} and \textit{bugs}, because there are both high and low WIP-limits when the correlations is almost the same. 

The literature states that when WIP-limit was too high, the number of bugs produced decreases. Based on these data, there is no evidence proving this statement. The data from this section shows that WIP-limits have no impact on the correlation between \textit{bugs} and \textit{bugs finished, quarter}, \textit{WIP} and   \textit{avg days in backlog, bugs}.
...
\label{sec:dis:churn}
Churn is used as surrogate for effort in this work. No literature was found where the relationship between \textit{churn} and \textit{WIP} are investigated. Kanban, as stated in Section \ref{sec:Kan}, suggests small tasks and constant flow of released features throughout the development.  In order to have frequent releases, the correlation between \textit{churn} and \textit{WIP} should be a positive one. The mean correlation between \textit{churn} and \textit{WIP} are -0.1, as showed by Table \ref{DS:corr:WIP:v2}.
There is one team with a positive significant correlation. That team has correlation of 0.77 and a WIP-limit of 3.21 as shown by Table \ref{corr:WIP:V2} and Figure \ref{Team:WIP:4}. The two other teams with the second and third highest correlations have the values of 0.39 and 0.16. These two teams have the WIP-limits of 2.31 and 0.71, showed by Table \ref{corr:WIP:V2} and Figures \ref{Team:WIP:2} and \ref{Team:WIP:8}. The correlations between these three teams are scattered and so are the WIP-limits. The three teams with the lowest WIP-limits have the correlation of -0.27, 0.39 and -0.37. The three teams with the highest WIP-limit have the correlation of 0.16, 0.77 and -0.35, showed by Table  \ref{corr:WIP:V2} and Figure \ref{table:WIP:teams}.  Based on these values, there is no pattern in the correlation between \textit{WIP} and \textit{churn}, because both high and low WIP-limits have high and low correlations between \textit{WIP} and \textit{churn}.

The mean correlation between \textit{bugs finished, quarter} and \textit{churn} are 0.5, which means that if the task effort increases, the number of bugs closed whiten the same quarter increases and vice versa. Based on the principle of Kanban, the task size should be minimized in order to deliver continuously. So the correlation between \textit{bugs finished, quarter} and \textit{churn} should be a negative one. The two \textbf{teams seven} and \textbf{nine} have a negative correlation of -0.16 and -0.12. These two teams have a mean WIP-limit of 2.03 and 2.19, as showed by Table \ref{corr:churn:v2} and Figure \ref{table:WIP:teams}. The two teams with the closest correlation to \textbf{team seven} and \textbf{team nine} have correlation of 0.34, 0.33 and WIP-limits of 3.21 and 3.02, showed by Table \ref{corr:churn:v2} and Figure \ref{table:WIP:teams}. The five teams with a positive significant correlations have WIP-limits of 0.73, 2.31, 1.78, 1.93 and 0.71, showed by Table \ref{corr:churn:v2} and Figure \ref{table:WIP:teams}. 
The teams with low WIP-limits and teams with high WIP-limits showed a pattern in their correlations, but the significant correlations have a scattered WIP-limits, this points towards the fact that the WIP-limits does not matter for the correlation between \textit{bugs finished, quarter} and \textit{churn}.
...
\section{Summary}
The correlation between \textit{lead time} and \textit{throughput} and \textit{lead time} and \textit{WIP} showed evidence that WIP-limits have an impact in software development. It is not certain that WIP-limit can control the correlation between \textit{lead time} and \textit{throughput} and \textit{lead time} and \textit{WIP}, there could be coincidences, because there is two correlation who showed a pattern between the correlation and WIP-limits. The correlation between \textit{lead time} and \textit{throughput} showed that the WIP-limits may be too high and the correlation between \textit{lead time} and \textit{WIP} showed that the WIP-limits may be too low. Based on this data, the WIP-limit should be between 2 and 3 in order to have a positive correlation between \textit{lead time} and \textit{WIP}. And in order to have lower the correlation between \textit{lead time} and \textit{throughput} the WIP-limit should be lower than 1.
Based on this results there could also be other variables that have an impact on the correlation between \textit{lead time} and \textit{throughput}, it is not sure that WIP-limit is the only variable that has an impact on this correlations, but this should be further investigated. 


\section{Discussion of the moderating variables}
Chapter \ref{ch:res} highlighted the variance between moderating variables for \textit{churn} and \textit{throughput}. The reason for the teams' variance is explained in the two following subsections.
...
\textbf{Team seven} has a significant correlation for \textit{throughput}, \textit{throughput feature}, but not \textit{throughput bug}, as showed in WIP correlation Table \ref{corr:WIP}. The table showed a correlation of 0.67 for \textit{throughput}, 0.63 for \textit{throughput feature} and 0.55 for \textit{throughput bug}. The difference between the correlations is small, which also can be assumed by the total row in Figures \ref{DS:Throughput:7},  \ref{DS:TPFT:7} and \ref{DS:TPB:7}. The total rows show \textit{throughput} has a mean of 2.7, while \textit{throughput feature} has a mean correlation of 2.8 and \textit{throughput bug} has a mean correlation of 2.6. \textit{Throughput feature} contributes 156 dates to \textit{throughput} and \textit{throughput bug} contributes 172 dates. Based on these numbers, it looks like both the moderating variables contribute. The \textit{throughput} correlation table in Section \ref{sec:corr:TP} proves the assumption with correlation of .91 for both \textit{throughput feature} and \textit{bug}.  
...
\chapter{Conclusion}
\label{ch:conc}
In this work the main goal was to investigate the research question: \textit{Does WIP-limit in software development matter?} If so: \textit{How can one find the optimal WIP-limit?} To answer the research questions, a data set from a software company called Software Innovation was interpreted. The data set is based on metadata about tasks from 2010 to 2013. More than ten mean correlations were investigated based on the WIP-limits. The correlation between \textit{throughput} and \textit{lead time} and \textit{WIP} and \textit{lead time} showed evidence that WIP-limits have an impact, but the rest of the correlations showed no impact of WIP-limits. 

One can see a pattern in the correlation between \textit{throughput} and \textit{lead time} and \textit{WIP} and \textit{lead time}. In the correlation between \textit{throughput} and \textit{lead time} one can see most of the teams with low mean WIP-limits have also low correlation, and most of the teams with high mean WIP-limit have high correlations. Although, there is no evidence showing that lowering WIP-limits even more will produce a negative correlation between \textit{throughput} and \textit{lead time}, but investigating this is suggested as further work. In the correlation between \textit{WIP} and \textit{lead time} the four highest correlations belonged to four of the five teams with a WIP-limit higher than 2, and three of these correlation were significant. The correlation between \textit{WIP} and \textit{lead time} do show that most of the teams with a mean WIP-limit of 2 or higher have at least a moderate correlation.

Based on these data, there is no optimal WIP for any given context. It depends if one wants to optimize the correlation between \textit{lead time} and \textit{throughput} or the correlation between \textit{lead time} and \textit{WIP}. So to answer: \textit{How can one find the optimal WIP-limit?} one should take the  correlations of \textit{lead time} and \textit{throughput} and \textit{lead time} and \textit{WIP} into  consideration when looking for the optimal WIP-limit based on these data.
...
There is also done another research on the data from SI, it is recommended comparing the result from this work against the other research. It is also recommended to try to decrease the WIP-limit to see if their current WIP-limit is too high, based on the correlation between \textit{lead time} and \textit{throughput}. It is also recommended to check the cycle time relationship for SI against the \textit{lead time} and \textit{WIP} correlation. It is also recommend to investigate if there is any other variables that plays a part in the correlation between \textit{lead time} and \textit{throughput}. It is also recommended using other statistical methods to falsify or verify the findings from this work.
...
\caption{DS - Bugs finished quarter}
 \label{DS:FTPQ:6}
\end{subfigure}
\end{adjustwidth}
\caption[Optional caption for list of figures]{Caption of Descriptive Statistic for Bugs and Bugs finished within quarter}
