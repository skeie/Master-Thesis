...
The Scrum process is the source of much of the thinking behind principles and values of the Agile Manifesto. Values as "Individuals and interactions over processes and tools", "Working software over comprehensive documentation", "Customer collaboration over contract negotiation" and "Responding to change over following a plan" relates directly to Scrum. \parencite{Scrum}.
...
\begin{enumerate}
\centering
\item \textbf{Start Early:}  Don't wait for details. As soon as enough information is gathered start the development activity. Don't wait for details, get everyone involved figure them out. Don?t build any walls, make people collaborate and start a two-way communication as soon possible. This will start the learning cycle as well.
...
The only thing worth doing is deliver value to the costumer, anything else is waste.  See waste and eliminate it is the first key of Lean.  Lean suggests using a value stream map for removing waste. A value stream map is a map over the whole company chain and helps visualize where waste are.
...
In the recent years, more software projects adapt to Kanban and Lean \parencite{DavidAnderson}, and this is one of the reasons why this thesis will focuses on Kanban and one of it's main tools WIP.
...
\label{chap:RQ}
In this thesis the overall research question will be to study the effects of WIP limits for SI. And to see if the research will give answers to the following question:
\begin{itemize} 
...
The explanation of how the program works will be split into sections so it will be easier to get a understanding of how the program operates. The explanation of how the program works will first contain a detailed description in words, then a description using Pseudocode \parencite{jd} will be provided. The Pseudocode are simplified with only the key elements of the program code. 

The following two tables (table \ref{table:variable} and table \ref{tab:measurementDone}) shows the five variables that will measured by the program and the latter table shows information about the quarters, dates and days.
\begin{table}[!ht]
\begin{center}
\begin{itemize}
\item WIP 
...
The data from SI will be analyzed using a program made for this thesis, which will compute the variables shown in table \ref{table:variable} for per day each of the six team. This data will be further analyzed using SPSS. With SPSS the data produced by the program will be interpreted. The interpreted data will be presented in chapter \ref{chap:res}. 
...
The Arraylist from section \ref{sub:stepOne}  and \ref{sub:stepTwo} now contain a WIP object for each date from 2008 to 2013 for each team. In this step the program will loop through the Arraylist, during the iteration each WIP object is extracted from the Arraylist and the new WIP is measured. The two methods stated in line ten and eighteen respectively gather the current WIP (method in line ten) and finds out how many finished tasks (method in line eighteen) and returns the result. The result is used in line five to compute the current WIP. 
...
As in the article "Quantifying the Effect of Using Kanban versus Scrum" \parencite{Dag} to prevent outliers from having a large effect the top and lowest ten percent of lead time and churn are removed from the data set. Since some tasks could be given low priority due to lack of manpower or the tasks is labeled as not critical.
