...
The correlation Table \ref{corr:WIP} shows \textbf{team one} has a positive correlation to \textit{throughput}, \textit{throughput feature}, \textit{bugs}, \textit{lead time}, \textit{churn feature} and \textit{team size}.

\textbf{Team two} has a significant negative correlation value to \textit{churn} and \textit{churn bug}. \textbf{Team three} has a positive correlation value to all \textit{throughput} variables and \textit{team size}. \textbf{Team four} has significant positive correlation to all variables except \textit{team size}. \textbf{Team five} has positive correlation to \textit{throughput bug}, \textit{lead time} and \textit{team size}.
...
There is a gap in the relationship between the \textit{churn} variables and \textit{throughput} variables for both team \textbf{one} and \textbf{six}. The \textit{throughput} variables for team \textbf{five, seven, eight} and \textbf{ten} also have a gap, the same goes for the \textit{churn} variables for team \textbf{team two},  showed in Table \ref{corr:WIP}.  The relationship between these variables are explained in Section \ref{sec:diss:sv}.
...
In Table \ref{DS:corr:WIP}, the mean correlation for \textit{throughput} and \textit{team size} is 0.6. The mean correlation value for \textit{lead time, throughput feature, throughput bug} is 0.5. The \textit{bug} variables have a mean correlation value of 0.4. The rest of the values have a mean value of 0.3 or less.
...
In Table \ref{corr:WIP:V2}, team \textbf{two} and \textbf{three} have a significant correlation value for two out of  three \textit{throughput} variables. \textbf{Team four} has a positive correlation for two of the \textit{churn} variables and \textbf{team eight} has significant correlation to one \textit{churn} variable. The relationships between these sub variables are explained in Section \ref{sec:diss:sv}. The descriptive statistic Table \ref{DS:corr:WIP:v2} shows that \textit{throughput} has a average correlation value of 0.4, based on all the teams throughput variable. The rest of the variables except \textit{throughputs} sub variables has an average correlation values 0.2 or less. 
...
Table \ref{corr:lt} displays the variances between the \textit{throughput} variables for team \textbf{one, two} and \textbf{eight}. There is also variance for the \textit{churn} variables for team \textbf{one, three, four} and \textbf{eight}. The relationships between these variables are explained in Section \ref{sec:diss:sv}. Table \ref{DS:corr:LT} displays the average correlation value of 0.5 for \textit{WIP} and \textit{throughput}. \textit{Throughput feature} has the average value of 0.4 and \textit{throughput bug, bugs, churn feature} and \textit{team size} have the average value of 0.3.
...
The \textit{throughput} relationship for team \textbf{five, six, eight} and \textbf{ten} show variances according to Table \ref{corr:LT:V2}. \textit{Churn} variables also show variances for team \textbf{one, two, four, five} and \textbf{eight}. The relationships between the variables are  explained in Section \ref{sec:diss:sv}.
...
\end{tabular}
 ...
 \end{table}
 \newpage
The second correlation table for bugs, showed in Table \ref{corr:Bugs:v2} shows \textbf{team one} has a significant correlation value to \textit{throughput bug}, \textit{bugs finished, quarter}, \textit{avg days in backlog, bugs} and \textit{lead time}. \textbf{Team two} has a significant correlation to all the \textit{throughput} variable and \textit{lead time}. \textbf{Team three} has a significant correlation value to all the \textit{throughput} variables and all the \textit{churn variables}. \textit{Team five} has a significant correlation value to all variables except \textit{WIP}, \textit{throughput feature}, \textit{bugs finished, quarter} and \textit{churn feature}.
...
\textit{Throughput} and \textit{churn} for team \textbf{five} and \textbf{eight} shows variance according to Table \ref{corr:Bugs:v2}.  Team \textbf{one} and \textbf{six} shows variance between the \textit{throughput variables}. The variables relationships are explained in Section \ref{sec:diss:sv}. Table \ref{DS:corr:Bugs:v2} displays a medium or strong relationship for \textit{throughput}, \textit{throughput bug}, \textit{bugs finished, quarter}, \textit{avg days backlog, days}, \textit{lead time} and \textit{churn bug}. 
...
\label{sec:corr:TP}
This section shows the correlation table between \textit{throughput} and the variables. The first correlation Table \ref{corr:TP} displays that \textit{Throughput} has a significant correlation to either \textit{throughput feature} or \textit{throughput bug} for each of the teams. The teams \textbf{three, four, five, seven} and \textbf{nine} have a positive correlation to both the \textit{throughput} sub variables. For team \textbf{one, six} and \textbf{eight} there is a variance between \textit{throughput feature} and \textit{throughput}.  For team \textbf{two} and \textbf{ten} is the variance between \textit{throughput bug} and \textbf{throughput}, according to Table \ref{corr:TP}. The relationships between the \textit{throughput} variables that have a variance are explained in Section \ref{sec:diss:sv}.


\textbf{Team one} has significant correlation to \textit{bugs, lead time, churn feature} and \textit{team size}. Team \textbf{three} has significant correlation to \textit{WIP, bugs} and \textit{lead time}. \textbf{Team four} has a significant correlation to \textit{WIP} and \textit{bugs}.  \textbf{Team five} has a significant correlation to \textit{bugs} and \textit{team size}.
...
\textbf{Team six} has a significant correlation to \textit{WIP} and \textit{team size}. \textbf{Team seven} has a significant correlation value to \textit{WIP}. \textbf{Team eight} has a significant correlation value to \textit{churn} and \textit{team size}. \textbf{Team nine} has significant correlation to \textit{bugs} and \textit{bugs finished, quarter}. \textbf{Team ten} has a significant correlation value to \textit{bugs}, \textit{bugs finished, quarter} and \textit{team size}. 


For team \textbf{one, four, six} and \textbf{eight} is there a variance between the \textit{churn} variables, the relationships between these variables are explained in Section \ref{sec:diss:sv}.
According to Table \ref{DS:corr:TP}, \textit{bugs} has a strong positive correlation with \textit{throughput} as stated Section \ref{sec:corr:bug}. \textit{WIP} has a strong positive relationship with \textit{throughput}, which were stated in Section \ref{sec:corr:WIP}. \textit{Team size} and \textit{lead time} has a medium-strong relationship to \textit{throughput}. The sub variables \textit{throughput feature} (0.8) and \textit{throughput bug} (0.7) has  a strong and \textit{throughput}. The rest of the values have the values of 0.3 or lower.
...
\newpage 
 ...
 \textbf{Team two} has significant correlation value with all variables except \textit{avg days in backlog, bugs} and the \textit{churn} variables.  \textbf{Team three} has a significant correlation value to \textit{WIP} and \textit{bugs}. \textbf{Team four} has significant correlation value to \textit{WIP}, \textit{bugs finished, quarter}, \textit{lead time}, \textit{churn} and \textit{churn feature}. \textbf{Team five} has a significant correlation value to all variables except \textit{WIP, throughput feature} and \textit{churn feature}. 

 
\textbf{Team six} has positive correlation value to \textit{lead time}. \textbf{Team eight} has a correlation value with \textit{bugs}, \textit{bugs finished, quarter, churn} and \textit{churn bug}. \textbf{Team nine} has a  significant correlation value to \textit{bugs}. \textbf{Team ten} has a significant correlation to \textit{bugs}, \textit{bugs finished, quarter} and \textit{lead time}. 
The \textit{churn} variables for team \textbf{four, five} and \textbf{eight} have variance, according to Table \ref{corr:TP:V2}. The \textit{churn} relationship is explained in Section \ref{sec:diss:sv}. Based on Table \ref{DS:corr:TP:v2}, \textit{throughput} has medium-strong relationship between \textit{WIP}, \textit{throughput} sub variables, \textit{bugs}, \textit{bugs finished, quarter} and \textit{lead time}.
...
The Table \ref{corr:churn} shows \textbf{Team one} has positive correlation to \textit{bugs}, \textit{bugs finished, quarter} and \textit{lead time}. \textbf{Team two} has a significant correlation value to \textit{WIP}. \textbf{Team four} has a significant correlation to all variables except \textit{bugs}, \textit{bugs finished, quarter}, \textit{avg days in backlog, bugs} and \textit{team size}. \textbf{Team five} has a negative correlation value with \textit{team size}. 
...
\textbf{Team one} has a significant positive correlation for \textit{bugs finished, quarter} and \textit{lead time}.  \textbf{Team two} has a significant correlation value with \textit{throughput feature}, \textit{bugs finished, quarter} and \textit{avg days in backlog, bugs}. \textbf{Team three} has a significant correlation value with \textit{bugs}, \textit{bugs finished, quarter}.  \textbf{Team four} has a significant correlation for \textit{WIP}, all \textit{throughput} variables, \textit{lead time}. \textbf{Team five} has a significant correlation value for \textit{throughput}, \textit{throughput bug}, \textit{bugs}, \textit{bugs finished, quarter}, \textit{avg days in backlog, bugs}, \textit{lead time} 
...
\end{tabular}
 ...
 \label{sec:corr:tz}
 The team size correlation Table \ref{corr:Teams} shows \textbf{team one} has a significant correlation value to \textit{WIP, throughput, throughput feature, bugs, lead time} and \textit{churn feature}. \textbf{Team two} has a correlation value with \textit{avg days in backlog, bugs}. \textbf{Team three} has a significant correlation value with \textit{WIP} and \textit{churn bug}. \textbf{Team five} has a significant correlation value to \textit{WIP, throughput, throughput bug, bugs, churn} and \textit{churn bug}.
...
Teams \textbf{one, five} and \textbf{six} have a variance for both \textit{throughput} and \textit{churn} variables, according to Table \ref{corr:Teams}. Teams \textbf{seven} and \textbf{ten} have a variance in the \textit{throughput} relationship, while \textbf{team three} has it for the \textit{churn} variables. The relationships between those variables are explained in Section \ref{sec:diss:sv}. The Table \ref{DS:corr:TS} shows a strong correlation between \textit{WIP}, \textit{throughput} and \textit{team size}, as well as a medium correlation for \textit{bugs}. 
