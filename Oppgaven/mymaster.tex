...
In order to see if WIP matters in agile method a raw dataset from SI will be analyzed. The SI set contains tasks recorded from 2008 to 2013.  The dataset contains various columns, but the columns of interest are listed in table \ref{IC}. In the table \ref{des} is a description of which columns the program needs in order to measure WIP, Throughput, Bugs, churn and Lead time.  
Since there is no software for analyzing the raw data, a program was made. A program will transform the raw data from SI into quantitative data, which will be analyzed using SPSS.  How the program will measure WIP will be described in section \ref{WPD},  Bug in section \ref{Bug}, Throughput in section \ref{TP} and churn in section \ref{churn}. The data set has already defined lead time. Information about how the program will measure WIP, throughput, bugs and churn will be explained in this chapter. The explanation of how the program works will be split into sections so it will be easier to get a understanding of how the program operates. The explanation of how the program works will first contain a detailed description in words, then a detailed description using Pseudocode\parencite{jd} will be provided. 
...
Table \ref {dataset} show an excerpt from the SI dataset.  The date 2008-10-09 is not in the set so the the program has to create 2008-10-09 and add it to the set in order to calculate WIP, throughput, bugs and lead time per day. 
...
\item All seven days in the week are taken into account when measuring WIP, throughput, bugs and lead time included Saturdays and Sundays
...
\item October. November and December (Q4)\parencite{Quarter}
\end{itemize}
\end{itemize}
\section{Swag}
The raw data from the SI set will first be ana
...
\subsection {Columns needed for calculating WIP, backlog and throughput}
\begin{table}[ht]
\begin{center}
...
\label{WPD}
The algorithm To describe the algorithm of measuring WIP limit per day, A detailed example on how the algorithm works is listed in the \ref{sec:Example}.

\subsubsection{Step 1: Gather all dates into a Hashmap}
\label{sub:stepOne}
...
return nr_of_date_occurrence
 \end{lstlisting}
\subsubsection{Step 2: Gather the remaining days}
 \label{sub:stepTwo}
...
\subsection{Example}
\label{sec:Example}
Figure \ref{wip_timeline}  shows tasks id in the y-axis and dates in the x-axis. The green line indicates the duration of the task. This table indicates how many tasks in progress (WIP). On 2008-10-12, tasks 3, 5 and 6 are in progress, which means WIP is 3 for 2008-10-12.  
\begin{figure}[ht!]
\centering
\hspace*{-2.2in}
\includegraphics[width=23cm]{Picture/wip_example.jpg}
...
\label{subsubsec:ft}
First the algorithm will do a look-up on the date 2008-10-07 referred with task ID one and two in table \ref{wt:2} and measure the current WIP counter, which is two, because its two tasks in progress on this date. After this the program will put the date and the current WIP into a hashmap. 
...
The program will do a look-up on the date 2008-10-10 referred with task ID six and seven in table \ref{wt:2}  and measure the current WIP for this date, which is two. Next the program will put the date and the current WIP into a hashmap.
...
In this master thesis function stated in figure \ref{SPSSfunctions} will be used to analyze data.
\begin{figure}[ht]
\begin{itemize}
\item Case summaries
\item Line Graph
