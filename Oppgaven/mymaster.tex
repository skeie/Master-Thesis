...
This section contains information about the correlation table between the variables and churn. Table \ref{corr:churn} shows the correlation  when team size was \textbf{not} taken into account. The table shows all teams have either one or both moderatingvariables with significant positive correlation with \textit{churn}. Teams \textbf{four, five, six, seven, eight, nine} and \textbf{ten} do not have a positive correlation between both the \textit{churn} moderating variables according to Table \ref{corr:churn}. The relationship between the \textit{churn} variables is explained in Section \ref{sec:diss:sv}. 
...
The second correlation table for \textit{churn}, Table \ref{corr:churn:v2} shows the \textit{churn} correlation table when team size is taken into account. The table shows all teams have either one or both moderating variables with significant positive correlation with \textit{churn}. Teams \textbf{one, four, five, six, seven, eight} and \textbf{ten} do not have a positive correlation between both the \textit{churn} moderating variables according to Table \ref{corr:churn}, the relationship between these variables is explained in Section \ref{sec:diss:sv}. 
...
The literature says that there should exist a negative correlation between \textit{WIP} and \textit{throughput}. The correlation from this work shows that almost each team has a positive correlation between \textit{WIP} and \textit{throughput} and a mean correlation of 0.4, as shown by Tables \ref{corr:WIP:V2} and \ref{DS:corr:WIP:v2}. The reason \textit{WIP} and \textit{throughput} have a positive correlation could be because the WIP-limits is set to low or too high. If the WIP-limits are too low, bottlenecks will not occur and the teams are able to produce more, but they are not allowed by the WIP-limit. If WIP-limits are too high, it can make people work on too many tasks which increase lead time for each task.

Section \ref{WIPsec} states that WIP-limits should be in the range from one to three tasks per developer. The mean WIP-limit for SI is 1.9 per developer, as stated by Section \ref{sec:WIP:limit}, which shows that the WIP-limit  could be either to low or too high. The rest of this section will explore the option that the WIP-limits may be set to low or too high and discuss if WIP-limits matter.  The correlation values of $\pm$ 0.2, showed by Table \ref{DS:corr:TP:v2} will be discussed. The mean correlations from 0.2 to -0.2 are discarded, because they are too low.


There exist two teams with a mean WIP-limit of 3 tasks per developer. These teams are  \textbf{team four} and \textbf{team six}, shown by Figures \ref{Team:WIP:4} and \ref{Team:WIP:6}. These two teams had a total number of 53 and 99 employees from 2010-4 to 2013-4, which gives a mean of 4 (53/14) and 7 (99/14) persons per quarter, showed by Tables \ref{Team:4} and \ref{Team:6}. There are no other teams with fewer employees per quarter than these two teams, although these two teams have a mean throughput of 6.2 for \textbf{team four} and 4.6 for \textbf{team six} as showed by the total mean row in the descriptive statistic Tables \ref{DS:TP:4} and \ref{DS:Throughput:6}. There are only two teams with a higher mean throughout than these two teams. The teams with a higher mean throughput are teams \textbf{one} and \textbf{five}, as shown by the total mean row in Tables \ref{DS:Throughput:1} and \ref{DS:Throughput:5}. In comparison between team \textbf{four} and \textbf{six},  \textbf{team one} had in average 24 (330/14) employees per quarter and \textbf{team five} had in average 20 (285/14) employees per quarter. 

\textbf{Team four} has a mean throughput of 1.4 per employee and \textbf{team six} has a mean throughput of 0.63 per employee, as shown by Tables \ref{Team:TP:4} and \ref{Team:TP:6}. The  team with the third highest  throughput mean per employee is \textbf{team two} with a mean of 0.44 and a mean WIP-limit of 2.31, showed by Figures \ref{Team:TP:2} and \ref{Team:WIP:2}. The teams \textbf{one} and \textbf{five} have a mean throughput of 0.42 and 0.30 as showed by Figures \ref{Team:TP:1} and \ref{Team:TP:5}. This shows that team \textbf{four} outperform the other teams on throughput per developer and \textbf{team six} is second by a wide margin. These data points towards the fact that WIP-limit may be too low, because the three teams with the highest throughput per employee also have the highest WIP-limits. 

The \textit{WIP} correlation Table \ref{corr:WIP:V2} shows \textbf{team two} with a correlation of 0.59 between \textit{throughput} and \textit{WIP}, \textbf{team four} with a significant correlation of 0.86 between \textit{throughput} and \textit{WIP} and \textbf{team six} with a correlation of 0.08 between \textit{WIP} and \textit{throughput}.  These correlations show that WIP-limit may not be too low, because \textbf{team two} and \textbf{team four} have positive significant correlation, and one is looking for a negative correlation. The teams with the lowest correlation between \textit{throughput} and \textit{WIP} are; \textbf{team ten} with a correlation of -0.21 and a WIP-limit of 1.08, \textbf{team five} with a correlation of 0.11 and a WIP-limit of 1.93 and \textbf{team six} with a correlation of 0.08 and a WIP-limit of 3.02 as showed in Table  \ref{corr:WIP:V2} and Figures \ref{Team:WIP:10}, \ref{Team:WIP:5} and \ref{Team:WIP:6}. Based on these values it will look like WIP-limits do not have an impact on the correlation between \textit{throughput} and  \textit{WIP}, because teams with both high and low WIP-limits have both high and low correlation between \textit{WIP} and \textit{throughput}.
...
The mean correlation between \textit{lead time} and \textit{throughput} are 0.5, as showed by Table \ref{DS:corr:WIP:v2}. The positive correlation between these variables states that when \textit{throughput} increases, the time from when a task is taken from the backlog until it is finished also increases.  This correlation relationship should be negative. Because if there are produced more tasks, the total time to produce tasks should be minimized. There are five teams with a positive significant correlation, as showed by \ref{corr:WIP:V2}. The mean WIP-limits for these five teams are 2.31, 3.21, 1.93, 3.02 and 1.08, as showed by Figure \ref{table:WIP:teams}. These WIP-limits and correlations show no pattern towards the fact that WIP-limits matter or that they are too high or low. \textbf{Team one} and \textbf{team eight} have the correlations of 0.06 and 0.24, as showed by Table \ref{corr:WIP:V2}. These correlations  are the two lowest between \textit{lead time} and \textit{WIP}.  \textbf{Team one} and \textbf{team eight} have the mean WIP-limits of 0.73 and 0.71, which are the two lowest mean WIP-limit, showed by Figures \ref{Team:WIP:1} and \ref{Team:WIP:8}. Three of the five teams with the significant correlations are also the three teams with the highest mean WIP-limits. This data shows that the highest mean WIP-limits have a significant correlation and the lowest mean WIP-limits are the one closes to have a negative correlation between \textit{throughput} and \textit{lead time}. Based on these data, it looks like WIP-limits matter and that the WIP-limits may be too high. 

The mean correlation between \textit{bugs} and \textit{throughput} are 0.6, as showed by Table \ref{DS:corr:WIP:v2}. The positive correlation between \textit{bugs} and \textit{throughput} states that if SI produces more, then they produce more bugs. This correlation relationship is standard. When one produces more tasks, there are likely to produce more bugs. If one produces zero tasks, one also produces zero bugs. In an optimal process should the correlation between \textit{bugs} and \textit{throughput} be a negative correlation, so teams can produce more without producing bugs. This is the principle of \textit{JIT}, explained in Section \ref{sec:JIT}.


There are six teams with a significant positive correlation between \textit{throughput} and \textit{bugs} as showed by Table \ref{corr:WIP:V2}. These six teams have a mean WIP-limit of 2.31, 1.78, 1.93, 0.71, 2.19 and 1.08, as showed by Figure \ref{table:WIP:teams}. This shows no pattern between \textit{WIP}-limits and the correlations between \textit{bugs} and \textit{throughput}. The teams \textbf{one} and \textbf{six} are the only teams with a correlation lower than 0.45. \textbf{Team one} has a correlation of 0.05 and a WIP-limit of 0.73. \textbf{Team six} has a correlation of -0.02 and a mean WIP-limit of 3.02, as showed by Table \ref{corr:WIP:V2} and Figure \ref{table:WIP:teams}.  These data strengthens the assumption that there is no pattern between \textit{WIP}-limits and the correlation between \textit{throughput} and \textit{bugs}, because teams with both high and low mean WIP-limits have significant correlation between \textit{throughput} and \textit{bugs}.  

The mean correlation between \textit{throughput} and \textit{bugs finished, quarter} are 0.4. The mean correlation between \textit{throughput} and \textit{throughput bug} are 0.8 and the mean correlation between \textit{throughput} and \textit{throughput feature} are 0.7, as showed by Table \ref{DS:corr:WIP:v2}. The positive correlation between  \textit{throughput} and \textit{bugs finished, quarter} states that when SI produces more, they are able to fix more bugs within the same quarter, which shows the same as the correlation between \textit{bugs} and \textit{throughput}. The positive correlation between \textit{throughput} and both the moderating variables  states that SI produces more bugs than feature. These correlations also show the same as the correlation between \textit{throughput} and \textit{bugs}. 


The mean correlation between \textit{throughput} and \textit{churn} are 0.3, as showed by Table \ref{DS:corr:WIP:v2}. The positive correlation states that if the tasks require more effort, they are able to produce more. This correlation is odd, the correlation between these two variables should be a negative one.  There are three teams with a significant correlation, as showed by Table \ref{corr:WIP:V2}. These three teams have the mean WIP-limit of 3.21, 1.93 and 0.71, showed by Figure \ref{table:WIP:teams}. There are three teams with a negative correlation between \textit{throughput} and \textit{churn}. These negative correlations are -0.14, -0.14 and -0.28, as showed by Table \ref{corr:WIP:V2}. The teams with these negative correlations have the mean WIP-limits of 0.73, 3.02 and 2.19, as showed by Figure \ref{table:WIP:teams}. Both teams with a high mean WIP-limit and low WIP-limit have both a significant positive correlation and negative correlation.  This states that there is no pattern between WIP-limits and the correlation relationship between \textit{throughput} and \textit{churn}. 

The data in this section shows that WIP-limit does not matter in the correlation for \textit{throughput}.  The correlation between \textit{throughput} and \textit{lead time} showed evidence that the WIP-limits could be too high and that the impact of WIP-limits do matter, but the other correlations showed no evidence of WIP-limits impact. 


%The Tables \ref{DS:corr:TP} and \ref{DS:corr:TP:v2} show no evidence towards the fact that lowering WIP-limit will increase \textit{throughput}. \textit{Throughput} has the mean correlation of 0.6 when team size is \textbf{not} taken into account and 0.4 otherwise. One can argue that the throughput correlation is biased when team size is \textbf{not} taken into account, based on the strong team size correlation. But the Table \ref{DS:corr:TP:v2}, where team size is taken into account shows a mean correlation  of 0.4 between \textit{throughput} and \textit{WIP} when team size is taken into account.

%When team size is \textbf{not} taken into account,  all of the teams have a positive correlation between \textit{WIP} and \textit{throughput}, and seven of the teams have a significant correlation, shown in Table \ref{corr:WIP}.  When team size is taken into account the Table \ref{DS:corr:WIP:v2}  shows nine out of ten teams have a positive correlation between \textit{WIP} and \textit{throughput}. %team was here
%The median between the teams when team size is \textbf{not} taking into account is 0.7 and 0.4 otherwise. The standard deviation is 0.2 when team size is \textbf{not} taking into account and 0.3 otherwise, shown by Tables \ref{DS:corr:WIP} and \ref{DS:corr:WIP:v2}. The median and standard deviation when team size is \textbf{not} taken into account shows most of the teams have a correlation of 0.7 or higher between \textit{WIP} and \textit{throughput}. When team size was taken into account the median and standard deviation showed that most of the teams have a correlation of 0.4 or higher, while the standard deviation shows that the values are more spread when team size is taken into account.
...
%The combined Table \ref{corr:All} shows a significant correlation of 0.41 when team size is \textbf{not} taking into account and Table \ref{corr:All:V2} shows an overall significant correlation  of 0.55 when team size is taken into account. The values presented in this section point towards the fact that when \textit{WIP} increases, so does \textit{throughput}. The WIP-limit for SI could be too low which explains why \textit{WIP} and \textit{throughput} have a positive correlation relationship.


%The \textit{throughput} values show that when team size is \textbf{not} taking into account, seven of the ten teams have a significant linear relationship and the mean \textit{throughput} value is 0.6. When team size is taken into account, four of the teams have a significant correlation and the mea mean is 0.4. These values shows that \textit{throughput} has a positive correlation relationship with \textit{WIP}. The two median numbers shows at least 50\% of the correlation values have a medium-strong correlation value. The standard deviation shows a low-medium spread between the values, which indicates that the values are in the same range. The overall correlation value when team size is \textbf{not} taking into account shows a positive medium-strong linear relationship, but when team size is taken into consideration the overall linear relationship is 0.05, but the mean for teams is 0.4 and four out of ten teams has a significant value.  Based on this, there is a low-medium linear relationship between \textit{WIP} and \textit{throughput}. 

%The two tables also show the moderating variables of \textit{throughput} with a mean of 0.5 when team size is \textbf{not} taken into account and 0.3 otherwise. This display that both the moderating variables  
...
The mean correlation between \textit{lead time} and \textit{churn} are 0.4, showed by Table \ref{DS:corr:LT:v2}. The teams should have a positive correlation between  \textit{lead time} and \textit{churn}, so the developers can work on small tasks and delivery quickly, which is stated in Section \ref{sec:Kan}.  Teams \textbf{one, four, five} and \textbf{eight} have a significant positive correlations and these teams have mean WIP-limits of 0.73, 3.21, 1.93 and 0.71, showed by Table \ref{corr:LT:V2} and Figure \ref{table:WIP:teams}. These values show no pattern, there are both teams with high and low WIP-limits with significant correlation. The three teams with the lowest correlations are the teams \textbf{six, nine} and \textbf{ten}. These teams have the WIP-limits of 3.02, 2.19 and 1.08, showed by Table \ref{corr:LT:V2} and Figure \ref{table:WIP:teams}. Based on the WIP-limits and the correlation, WIP-limits cannot be used to regulate the relationship between  \textit{lead time} and \textit{churn}, because there is no pattern between WIP-limits and the correlation values.
...
%The result from when team size was taken into account shows a mean correlation relationship of 0.2, which is a weaker correlation relationship. Three of the ten teams have a significant positive correlation, while the rest of teams' correlation vary from 0.32 to -0.18, as shown by Tables \ref{corr:WIP:V2} and \ref{DS:corr:WIP:v2}. 
...
%The combine correlation Table \ref{corr:All} shows that all teams combined have a significant correlation of 0.27, when team size is \textbf{not} taken into account.  The correlation Table \ref{corr:All:V2} shows a significant correlation of 0.36 when team size is taken into account. The value presented in this section shows when time size is taken into account, the mean correlation is 0.2, but the combine correlation is a significant one of 0.36. This shows a low to medium positive correlation relationship between \textit{WIP} and \textit{lead time}. When team size is \textbf{not} taken into account; there is a low significant correlation for all the teams combined and medium-high positive correlation when each team is measured independently. Based on these results, decreasing the WIP-limit will decrease \textit{lead time}.
...
The correlation between \textit{bugs} and \textit{bugs finished, quarter} should be positive. Teams \textbf{one, six, seven, eight} and \textbf{ten} have a significant positive correlation between \textit{bugs} and \textit{bugs finished, quarter}. These teams have a mean WIP-limit of 0.73, 3.02, 2.03, 0.71 and 1.08, as shown by Table \ref{corr:Bugs:v2} and Figure \ref{table:WIP:teams}. These values show no pattern between \textit{WIP-limit} and the correlation between \textit{bugs} and \textit{bugs finished, quarter}. There are two teams with a correlation lower than 0.20 and these teams have the WIP-limit of 3.21 and 2.19. The three remaining teams have the correlation of 0.44, 0.50 and 0.52, showed by Table \ref{corr:Bugs:v2} and have the WIP-limits of 2.31, 1.78 and 1.93, as showed by Figures \ref{Team:WIP:2}, \ref{Team:WIP:3} and \ref{Team:WIP:5}.  The WIP-limit for the teams with the correlation of 0.44, 0.50 and 0.52 shows some sort of a pattern, these WIP-limits are close.
...
%The result from this work shows when team size is \textbf{not} taken into account, the mean correlation between \textit{WIP} and \textit{bugs} are 0.5 and four of the teams have a significant positive correlation between \textit{bugs} and \textit{WIP}. All of the ten teams also have a positive correlation, as shown in Table \ref{corr:WIP}. The two variable \textit{bug finished, quarter} and \textit{avg days backlog bugs} is used as a side measures. The two variables are used to see how long bugs are in backlog and the amount of bugs fixed vs. the amount of bugs recorded in system per quarter. The mean correlation between \textit{WIP} and both \textit{bug finished, quarter} is 0.3 and \textit{avg days backlog, bugs} is 0.1. The value \textit{bug finished, quarter} shows a low-moderate mean correlation relationship. The mean correlation of 0.1, the combine correlation of 0.17 from Table \ref{corr:All} and the fact that there is one team with a significant correlation relationship between \textit{avg days backlog, bugs} and \textit{WIP} shows that  \textit{WIP}-limit has no influence on the number of days bugs are in backlog, when \textit{team size} is \textbf{not} taken into account. The \textit{bugs finished, quarter} with the mean of 0.3, the significant positive correlation of 0.17 between \textit{WIP} and \textit{bugs finished, quarter} from Table \ref{corr:All} and \textit{bugs}, shows a weak correlation relationship between  and \textit{WIP}. 

%When team size is taken into account, the mean correlation is 0.2 for \textit{bugs}, as shown by Table \ref{DS:corr:WIP:v2}.  Eight out of ten teams have a positive correlation relationship, but only one team has a significant positive correlation. The  \textit{bug finished, quarter} and \textit{avg days backlog, bugs}  have the mean correlation  0.2 and -0.1. For \textit{bug finished, quarter} is there four teams with a positive significant correlation and one team with a negative  significant correlation. For \textit{avg days backlog, bugs} is there one team with a negative significant correlation. The combine Table \ref{corr:All:V2} shows \textit{bugs finished, quarter} with a significant correlation of 0.40 and \textit{avg days in backlog, bugs} with a negative correlation of -0.14. These values show a weak correlation relationship between \textit{bugs} and \textit{WIP}, a moderate significant correlation relationship between \textit{WIP} and \textit{bugs finished, quarter} and a weak correlation between \textit{WIP} and \textit{avg days backlog, bugs}.

%The standard deviation when team size is \textbf{not} taken into account is 0.2 and the median is 0.5. The median when team size is taken into account is 0.2 and the standard deviation 0.4. The median numbers show significant impact of team size between the relationship of \textit{WIP} and \textit{bug}, as shown in \ref{DS:corr:WIP} and \ref{DS:corr:WIP:v2}    

%When team size is \textbf{not} taken into account, the overall correlation is significant with a value of  0.41, shown in Table \ref{corr:All}.  The correlation Table \ref{corr:All:V2} shows the correlation from when time size is taken into account. The table shows an overall significant correlation  of 0.29. The values from this section shows there is evidence that WIP-limit helps decrease bugs.



\section{WIP and Churn}
Churn is used as surrogate for effort in this work. No literature was found where the relationship between \textit{churn} and \textit{WIP} were investigated, Kanban, as stated in Section \ref{sec:Kan}, suggests small tasks with frequently releases.  The mean correlation between \textit{churn} and \textit{WIP} are -0.1, as showed by Table \ref{DS:corr:WIP:v2}. In order to have frequently releases, the correlation between \textit{churn} and \textit{WIP} should be a positive one. There is one team with a positive significant correlation. That team has the correlation of 0.77 and a WIP-limit of 3.21 as shown by Table \ref{corr:WIP:V2} and Figure \ref{Team:WIP:4}. The two other teams with the second and third highest correlations have the value of 0.39 and 0.16. These two teams have the WIP-limits of 2.31 and 0.71, showed by Table \ref{corr:WIP:V2} and Figures \ref{Team:WIP:2} and \ref{Team:WIP:8}. The correlations between these three teams are scattered and so are the WIP-limits. The three teams with the lowest WIP-limits have the correlation of -0.27, 0.39 and -0.37. The three teams with the highest WIP-limit have the correlation values of 0.16, 0.77 and -0.35, showed by Table  \ref{corr:WIP:V2} and Figure \ref{table:WIP:teams}.  Based on these values, there is no pattern between WIP-limit and \textit{churn}, because both high and low WIP-limits have high and low correlations between \textit{WIP} and \textit{churn}.

The mean correlation between \textit{bugs finished, quarter} and \textit{churn} are 0.5, which means that if the effort of tasks increases, the number of bugs closed whiten the same quarter increases. Based on the principle of Kanban, the task size should be minimized in order to deliver continuously. So the correlation between \textit{bugs finished, quarter} and \textit{churn} should be a negative one. The two teams \textbf{seven} and \textbf{nine} have a negative correlation of -0.16 and -0.12. These two teams have a mean WIP-limit of 2.03 and 2.19, as showed by Table \ref{corr:churn:v2} and Figure \ref{table:WIP:teams}. The two teams with the closest correlation to \textbf{team seven} and \textbf{team nine} have the correlation of 0.34, 0.33 and WIP-limits of 3.21 and 3.02, showed by Table \ref{corr:churn:v2} and Figure \ref{table:WIP:teams}. The two teams with the highest correlations of 0.95 and 0.90 have WIP-limits of 0.73 and 1.93, showed by Table \ref{corr:churn:v2} and Figure \ref{table:WIP:teams}. When WIP-limit was two per developer, two teams have a negative correlation and one team has a significant correlation between \textit{WIP} and \textit{churn}. When the WIP-limit was three, there were two teams with a correlation of 0.34 and 0.33. This points towards the fact that the WIP-limits does not matter, since the same WIP-limits both have high and low correlations.
