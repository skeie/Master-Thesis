...
\hline
 ...
 Leadtime  & -0.09 & 0.67** & -0.03 & 0.87** & 0.03 & 0.32 & 0.77* & -0.09 & -0.18 & -0.05 \\ \hline
...
\end{tabular}
 ...
 \end{table}
 \newpage
In Table \ref{corr:Bugs:v2} has Team one a significant positive correlation for \textit{throughput bug}, \textit{bugs finished, quarter} \textit{avg days in backlog, bugs} and \textit{lead time}, but not \textit{throughput}. The \textit{throughput} relationship were stated in Section \ref{sec:corr:WIP}. Team two has a positive correlation for the \textit{throughput} variables and \textit{leadtime}. Team three has a positive correlation for \textit{throughput} variables and \textit{churn} variables.  Team five has a significant positive correlation between \textit{avg days in backlog, bugs}, \textit{lead time}, \textit{churn}, \textit{churn bug}, \textit{throughput} and \textit{throughput bug}. The \textit{churn} relationship is stated in Section \ref{sec:corr:lt}, while the \textit{throughput} relationship were stated in Section \ref{sec:corr:WIP}. 
\FloatBarrier
\begin{table}[H]
  \begin{adjustwidth}{-1.5cm}{}
 \centering
...
\hline
 ...
 Leadtime  & 0.83** & 0.72** & 0.48 & 0.22 & 0.72** & 0.18 & 0.85** & -0.23 & 0.54 & 0.88** \\ \hline
...
\hline
 ...
 Leadtime  & 0.06 & 0.88** & 0.26 & 0.90** & 0.80** & 0.69** & 0.33 & 0.24 & 0.32 & 0.90** \\ \hline
...
\hline
 ...
 Leadtime  & 0.72** & 0.49 & 0.16 & 0.96** & 0.94** & 0.04 & 0.08 & 0.71** & -0.18 & 0.01 \\ \hline
...
\hline
 &  \bf{WIP} & \bf{TP} & \bf{TP ft} & \bf{TP bug} & \bf{Bugs} & \bf{Bugs, qrt} & \bf{Avg backlog bugs} & \bf{Lead time} & \bf{Churn} & \bf{Churn ft} & \bf{Churn bug}\\ \hline
...
\chapter{Discussion}
\label{ch:dis}
In this chapter a discussion around the research questions, the different results and what literature says are presented. 

\section{WIP and lead time}
\vspace{-1em}
As stated in Section \ref{sec:in:lt}, lead time could be important to measure productivity. Each development process would like to get their lead time as low as possible. In Section \ref{WIPsec}, there is stated that WIP-limits are important to reduce lead times. The same is showed by the results here. The result from when team size was \textbf{not} taken into account shows a linear relationship of 0.5, as shown in Table \ref{DS:corr:WIP}, which means when WIP increases, so does lead time. Team one, four, five, seven and ten has significant positive linear relationship and both team two and three has a medium correlation relationship between \textit{WIP} and \textit{lead time}  as shown in Table \ref{DS:corr:WIP}. 

The result from when team size was taken into account is a linear value 0.2, whichs shows a weaker linear relationship than when team size was \textbf{not} taken into account. Three of the ten teams have a significant positive correlation. While the rest of teams correlation values varies from -0.18 to 0.32, as shown by Tables \ref{corr:WIP:V2} and \ref{DS:corr:WIP:v2}. 

The standard deviation shows a variation of 0.2, and a median of 0.5 when team is \textbf{not} taken into account. The standard deviation when team size is taken into account is 0.4 and the median is 0 as shown in Tables  \ref{DS:corr:WIP} and \ref{DS:corr:WIP:v2}. The standard deviation shows a high proliferation of values, which shows evidence of spread relationship between \textit{WIP} and \textit{lead time} across the teams, when team size is taken into account.   The standard deviation value of 0.2 shows that  each of the ten \textit{lead time} values are closer when team size is \textbf{not} taken into account.  The median of the two results is 0.5 and 0. These values shows how the overall values when team size is \textbf{not} taken into account is greater. But, the max value for when team size is taken into account is 0.9 while 0.7 otherwise. This shows, isolated for that team what an impact increased WIP has.    


Despite \textit{WIP} and \textit{lead time} don't has a close relationship,  \textit{Lead time} has a close positive relationship to four of the five individual variables, as shown in table \ref{DS:corr:LT:v2}. \textit{Throughput}, \textit{Bugs} and  \textit{Bugs finished, quarter}  has the total correlation value 0.5, while \textit{churn} has the value of 0.4, when time size is taken into account. When team size is \textbf{not} taken into account, \textit{WIP} and \textit{throughput} has the overall correlation value of 0.5 and \textit{throughput feature} has the value 0.4, while the rest variables has a value of 0.3 or less. Especially, when team size is taken into account, the importance of decreasing \textit{lead time} is shown.

\section{WIP and bugs}
To minimize bugs should be a goal independent of software process and methods. If WIP-limit could reduce bugs without compromising, one could conclude that WIP-limit matter in software development. In the article by Shinkle \parencite{Shinkle}, when WIP was too high, the number of bugs increased. The same can be said about the values from this study. When team size is \textbf{not} taken into account, the correlation value between \textit{WIP} and \textit{bugs} is 0.4 and four of the teams have a significant positive correlation value between \textit{bugs} and \textit{WIP}. All ten teams has a positive correlation value, as shown in table \ref{corr:WIP}. The two variable \textit{bug finished, quarter} and \textit{avg days backlog bugs} is used as a side measure to how long bugs are in backlog and the amount of bugs fixed vs. the amount of bugs recorded in system per quarter. The correlation value for \textit{bug finished, quarter} is 0.3 and \textit{avg days backlog bugs} is 0.1. The value \textit{bug finished, quarter} shows a low-medium linear relationship who indicates that if WIP increases, SI is able to finished more bugs. One can argue that when one produces more, more bugs are produced, but the value of  \textit{bug finished, quarter} is measured in percentages.
The 0.1 correlation value of  \textit{avg days backlog bugs}  shows that \textit{WIP} has no influence on the number of days bugs are in backlog. But both \textit{bugs} and \textit{bug finished, quarter} shows a medium linear relationship, which shows moderate linear relationship between \textit{WIP} and these variables.

When team size is taken into account, the correlation value is 0.2, as shown in Table \ref{DS:corr:WIP:v2}.  Eight out of ten teams have a positive correlation relationship, but only one team has a significant value as shown in Table \ref{corr:WIP:V2}. The values \textit{bug finished, quarter} and \textit{avg days backlog bugs}  has the correlation values 0.2 and -0.1 This shows that there is small linear relationship between \textit{WIP} and any of the \textit{bugs} variables, when team size is taken into account. 

The standard deviation when team size is \textbf{not} taken into account is 0.2 and the median is 0.5. The median when team size is taken into account is 0.2, while the standard deviation is increased by 0.1. The median numbers shows significant impact of team size between the relationship of \textit{WIP} and \textit{bug}. The std.dev value shows a minor variation between the two results variables.  
...
\vspace{-0.5em}
In this thesis, throughput is a measure to see how productive teams are. In Section \ref{WIPsec}, there was stated by various people, when lowering WIP-limit, throughput increases. The data from this study shows otherwise. Both Tables \ref{DS:corr:TP} and \ref{DS:corr:TP:v2} proves that if \textit{WIP} increase, so does \textit{throughput} with the correlation value of 0.6 and 0.4. In Table \ref{DS:corr:TP}, one can argue that the throughput correlation value is biased, based on the strong team size correlation. But the Table \ref{DS:corr:TP:v2}, where team size is taken into account shows a overall correlation value of 0.4 between \textit{throughput} and \textit{WIP}. This shows that still when team size is taken into account, \textit{WIP} and \textit{throughput} still has a positive correlation. 

When team size is \textbf{not} taken into account,  All of teams have a positive correlation between \textit{WIP} and \textit{throughput}, and seven of the teams have a significant correlation value, as shown in table \ref{corr:TP}.  When team size is taken into account, the Table \ref{DS:corr:WIP:v2}  shows when \textit{WIP} increases so does \textit{throughput} for nine out of ten teams. Team ten is the only team with a negative correlation.  Team, five, six, eight and ten has low small correlation between \textit{throughput} and \textit{WIP}, Team one and seven has a low-medium relationship, team two, three, four and nine has a significant linear relationship. The \textit{throughput's} sub variables both have a overall correlation of 0.5 when team size is \textbf{not} taken into account. When team size is taken into account \textit{throughput feature} has a value of 0.3 and \textit{throughput bug} has the value 0.4. This shows that both \textit{throughput's} sub variables have a positive linear relationship with \textit{WIP}.

The correlation between \textit{throughput} and \textit{WIP} when team size is \textbf{not} taken into account is a strong linear relationship, the correlation value when team size is taken into account represents a medium linear relationship. The strong relationship means when \textit{WIP} increases, \textit{throughput} increases. The medium relationship means that when \textit{WIP} increases, \textit{throughput} increase as well to an extent. The throughput descriptive statistic Tables \ref{DS:corr:TP} and \ref{DS:corr:TP:v2} shows \textit{Bugs} has the overall correlation value of 0.6 and \textit{lead time} has the overall value of 0.5 with \textit{throughput}. One could see that if \textit{throughput} increases, so does both \textit{lead time} and \textit{bugs}. Both these values can't be used to see the impact of WIP-limits. These variables are isolated and can only be used to see what impact \textit{throughput} has on a software process.

The median when team size is \textbf{not} taking into account is 0.7 and 0.4 otherwise. The std.dev is 0.2 when team size is \textbf{not} taking into account and 0.3 otherwise.  A median of 0.7 is high, while 0.4 also relative high and when the std.dev is at the lowest shown in Tables \ref{DS:corr:WIP} and \ref{DS:corr:WIP:v2}, the relationship between \textit{WIP} and \textit{throughput} is significant.        



\section{WIP and Churn}
Churn is used as surrogates for effort in this thesis. It turns out based on the results, independently of  team size, there is no relationship between \textit{WIP} and \textit{churn} or the sub variables of churn. Both the results shows respectively a overall correlation of -0.1 in Table \ref{DS:corr:WIP:v2} and 0 in Table \ref{DS:corr:WIP}. When  team size is \textbf{not} taken into account \textit{Churn} has no linear relationship between any of the other variables, if one disregards \textit{churn's} sub variable shown in Table \ref{DS:corr:Churn}. When  team size is taken into account, \textit{bugs finished, quarter} has a strong relationship with a value of 0.6, while \textit{lead time} has a value of 0.4 which is a medium-strong relationship, shown in Table \ref{DS:corr:churn:v2}.

The std.dev for both the results are 0.4, while the median is 0.1 when team size is \textbf{not} taken into account and -0.1 for the other case. The std.dev empower the assumption of a relationship between \textit{churn} and \textit{WIP}, since the values are spread and their is no pattern.  


\section{Team size}
As the two different results sections in Chapter \ref{ch:res} and the sections above shows, it's important to take team size into account when looking for relationship between variables such as \textit{WIP}, \textit{throughput}, \textit{lead time} and \textit{bugs}. If team size were left out as a variable in this case study, \textit{WIP} would had a linear relationship of 0.6 with \textit{throughput}, 0,4 with \textit{bugs} and 0.4 with \textit{lead time}. The three correlation values increased by 0.2 compared to when  team size was taken into account. These variables would have showed stronger results towards the fact that WIP-limit matter in software development. 

For \textit{throughput} when time size was \textbf{not} taken into account, \textit{bugs} has a value of 0.6 and \textit{lead time} has a value of 0.5, which indicates a medium - strong relationship. The median in \textit{bugs} case is 0.8 and for \textit{lead time} it's 0.5 as shown in Tables \ref{DS:corr:TP} and \ref{DS:corr:TP:v2}. This values shows that when throughout increases, do does \textit{bugs} and \textit{lead time}. When team size is taken into account, the correlation values between \textit{throughput} and both \textit{bugs} and \textit{lead time} is the same , the std.dev and median for \textit{bugs} is different.  If \textit{WIP} omitted, the correlation relationship between the other variables and \textit{throughput} don't differ to much. This show evidence that team size don't has a big impact on the correlation relationship between variables and \textit{throughput} and team size.  

The bugs correlation tables \ref{DS:corr:Bugs} and \ref{DS:corr:Bugs:v2} shows a difference in the correlation value between \textit{bugs} and \textit{WIP}. The correlation value is increased by 0.2 when team size is \textbf{not} taken into account. Both the \textit{throughput} correlation values are 0.6. The correlation value for \textit{lead time} is also increased by 0.2 when team size is \textbf{not} taken into account, while churn increased by 0.3. These values show an impact of team size on the correlation relationship for 3 of 4 variables. 


For \textit{lead time} increases the churn value by 0.2 when time size is \textbf{not} taken into account, otherwise are the correlation values for \textit{lead time} stated preceding paragraphs. The values show the importance of taking team size into account when a case study like this is conducted.

\section{WIP}
The article "Studying Lean-Kanban Approach Using Software Process Simulation" \parencite{DavidAnderson} simulated a  lean-kanban process as stated in \ref{WIPsec}. The simulation showed when WIP was limit, the software process used 100 days and 120 otherwise. The article "Simulation of software maintenance process, with and without a work-in-process limit" \parencite{SMR:SMR1599} showed that WIP-limit can lead to improvement in throughput. The article also showed that when taking a process without WIP-limit, and simulate it using WIP-limit it outperformed the original process. The result from this thesis shows that \textit{throughput} increases when \textit{WIP} does. \textit{Throughput} is also the only variable with a correlation relationship of interest. The other correlation relationship is equal or lower than 0.2.  
 
\chapter{Conclusion}
\label{ch:conc}
In this thesis the main goal was to investigate the research question "Does WIP limit in software development matter?".  If so, further goals were to see "How to find the optimal WIP limit" and "Which parameters should be considered in order to optimize WIP".  To answer the research questions a data set from a company called Software Innovation were interpreted. The data set is based on metadata about tasks from 2010 to 2013.

If WIP limit matters in software development some of its benefits should be to reduce bugs, increase throughput, decrease lead time etc. Based on the results from the case study, \textit{WIP} has an impact on \textit{throughput}. The results shows a positive linear relationship between \textit{WIP} and \textit{throughput}. The previous research has stated to lower WIP to decrease throughput. This research shows otherwise. This study shows \textit{throughput} decreases when \textit{WIP} does. It is also not evidence after this case study that lowering WIP increases \textit{throughput} or decreases \textit{lead time} or \textit{bugs}.  Since WIP - limits don't matter in software development based on this study, there is no need to find an optimal WIP-limit or know which parameters to take into account in order to optimize WIP. 
...
%write about taken all the data and the correlation, not split by teams. 
