...
The study also showed that limit WIP can improve throughput and work efficiency. In the article, they authors also simulated a software process with who was originally without WIP - limits, with WIP-limits. The original process was outperformed by the simulated process with WIP-limits. \parencite{SMR:SMR1599}.

The case by David Anderson et al. \parencite{DavidAnderson} did a simulation of lean-kanban approach with the impact of WIP limit vs. no WIP limit on developers with skills in different activities. The four skill activities from the article where design, development, testing and deployment. 
...
\item Kanban WIP-limits are important to reducing lead times \parencite{KanbanWay} 
\end{enumerate}
...
Each section except team size is presented with two correlation tables and two corresponding descriptive statistics tables, one correlation table and descriptive statistic table for the first analyze and the same two tables are presented for the second analyze. \textit{Team size} is presented with one correlation table and one descriptive statistic table. The content of the sections will consist of highlighting the variables with a significant correlation and variables that stands out. To back up any assumptions about the variables, descriptive statistic tables listed in Appendix \ref{app:DS} and correlation graphs will be used. 

\section{Correlation - WIP}
\label{sec:corr:WIP}
Table \ref{corr:WIP} shows the correlation table for WIP. The variables are shown vertically in the correlation table. Horizontally are the corresponding teams. The team names are shortened. Team one is shortened to T1, team two is shortened to T2 and so on.
...
Team ten has significant positive relationship between \textit{WIP} and both \textit{throughput} and \textit{throughput bug}, but not for \textit{throughput feature}. \textit{Throughput} for team ten consist of 404 dates as show in the total row in Table \ref{DS:Throughput:7}. \textit{Throughput bug} represents 335 of these dates, while \textit{throughput feature} stands for 69 dates as shown in Tables \ref{DS:TPFT:7} and \ref{DS:TPB:7}. But the overall mean for \textit{throughput}, \textit{throughput feature} and \textit{throughput bug} is 2.2, 2.3 and 2.2, which could reflect a close relationship between these three variables. Both the throughput correlation table in Section \ref{sec:corr:TP} and the correlation graphs in Figures \ref{corr:Difference:10} disproves that assumption. The Figures \ref{fig:a:WIP:10} shows a vague significant positive correlation, while Figure \ref{fig:b:WIP:10} shows a significant positive correlation. The correlation table shows  \textit{throughput bug} with a correlation of .98 and \textit{throughput feature} with a correlation of .43.  Which proves that \textit{throughput bug} represents most of \textit{throughput}. 
...
For team three is \textit{throughput bug} close to a significant positive correlation with a value of 0.52, while both \textit{throughout} and \textit{throughput feature} has the values .57 and .71. With these correlation values, it looks like \textit{throughput bug} represents most of the \textit{throughput} variable, even through \textit{throughput bug's} value is not a significant. The descriptive statistic tables empower the assumption. \textit{Throughput} contains 542 dates and has a total mean value of 3.7. \textit{Throughput feature} represents 200 of these dates, and has a mean value of 3.3, while \textit{throughput bug} represents the remaining 342 dates, and has a total mean value of 4.0, shown in tables \ref{DS:Throughput:3}, \ref{DS:TPFT:3} and \ref{DS:TPB:3}. The \textit{throughput} correlation Table \ref{corr:TP:V2} proves the assumption with a correlation value of 0.90 for \textit{throughput feature} and a value of 0.98 for \textit{throughput bug}

Team four has a signification positive correlation for all the \textit{throughputs} variables, \textit{bugs finished, quarter}, \textit{lead time}, \textit{churn} and \textit{churn feature}.  The reason churn bug differ is stated in section \ref{sec:corr:lt}. For team two differ \textit{throughput feature} based on the correlation values in  \ref{corr:WIP:V2}. The reason for this is stated in Section \ref{sec:corr:bug}. For team eight differ \textit{churn feature} from \textit{churn} based on the correlation value in Table \ref{corr:WIP:V2}. The reason is stated in Section \ref{sec:corr:lt}. Team nine has a significant positive correlation for all \textit{throughput} variables. Team nine also has a significant positive correlation for all the \textit{throughput} variables. In Table \ref{DS:corr:WIP:v2}, \textit{throughput} has a overall correlation value of 0.4, while rest of the variables except \textit{throughputs} sub variables has a correlation values 0.2 or less. 
...
Team five has significant correlation value for \textit{bugs}, \textit{bugs, finished, quarter}, \textit{avg days in backlog, bugs}, \textit{churn}, \textit{churn bug}, \textit{throughput} and \textit{throughput bug}. The relationship between the \textit{throughput} variables were stated in Section \ref{sec:corr:WIP}, while the \textit{churn} relationship is explained in Section \ref{sec:corr:churn}. Team six has a significant correlation for \textit{throughput} and \textit{throughput feature}. The \textit{throughput} relationship were explained in  Section \ref{sec:corr:WIP}. Team seven has a significant correlation for \textit{WIP}, \textit{bugs} and \textit{bugs, finished, quarter}. Team eight has a significant correlation for \textit{throughput feature}, \textit{churn} and \textit{churn feature}. The \textit{throughput} relationship is explained in Section \ref{sec:corr:TP}, while the \textit{churn} relationship is explained in Section \ref{sec:corr:churn}. Team ten has a significant positive correlation for \textit{throughput}, \textit{throughput bug} and \textit{bugs}. The \textit{throughput} relationship were explained in Section \ref{sec:corr:WIP}.  The Table \ref{DS:corr:LT:v2} Shows a medium or strong relationship for \textit{throughput} variables, \textit{bugs}, \textit{bugs finished, quarter} and \textit{churn}. 
...
\label{sec:corr:bug}
This section contains information about the correlation table between the variables and \textit{bugs}. The result is listed in Table \ref{corr:bug}. Teams one, two, three, four, five, nine and ten all have a significant positive correlation between \textit{bugs} and \textit{throughput}. For team three, four, five and nine's case, all \textit{throughput} variables have a significant positive correlation with \textit{bugs}. For team one and ten, the \textit{throughput} variables differ. The \textit{throughput} relationships were stated in Section \ref{sec:corr:WIP}. 
 
Team one, three, four and nine has a significant positive correlation for \textit{WIP}.  Team one also has a significant positive correlation for \textit{lead time}, \textit{churn}, \textit{churn feature} and \text{team size}. The \textit{churn} relationship for team one were stated in Section \ref{sec:corr:WIP}. Team five has a significant positive correlation for \textit{team size} while \textit{team eight} has a significant positive correlation for \textit{churn bug}. The \textit{churn} relationship were explained in Section \ref{sec:corr:lt}.
\begin{table}[!htbp]
 \begin{adjustwidth}{-2.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
Team six, seven and nine has a significant positive correlation between the variable \textit{Bugs finished, quarter} and \textit{lead time}. Team nine also has a significant negative correlation between \textit{bug} and \textit{churn feature}, but not \textit{churn}. \textit{Churn feature} represents 201 of 538 dates as shown in Tables \ref{DS:Churn:9} and \ref{DS:CF:9}.  The total mean of \textit{churn feature} is 115.9, the total mean for \textit{churn} is 72.2 and the total mean for \textit{churn bug} is 46.1, as shown in the previous stated tables and Table \ref{DS:CB:10}. Judging from these numbers, both \textit{churn feature} and \textit{churn bug} contribute to \textit{churn}. The Figure \ref{corr:Difference:9} shows the correlation values in a graph. There is hard to find any pattern or evidence that \textit{churn feature} and \textit{churn bug} contribute to \textit{churn}. But, The churn correlation Table \ref{corr:churn} shows \textit{churn feature} correlation values is .60 and \textit{churn bug} correlation value is 0.39. This proves that both the sub variables of \textit{churn} contribute, with \textit{churn feature} contributing slightly more . 
...
Team six has a significant positive correlation for \textit{throughput bug} and \textit{bugs finished quarter}. The \textit{throughput} relationship were explained in Section \ref{sec:corr:WIP}. Team seven has a significant positive correlation for \textit{WIP}, \textit{bugs finished, quarter} and \textit{lead time}. Team eight has a significant positive correlation for \textit{throughput}, \textit{throughput bug}, \textit{bugs finished, quarter} and \textit{churn bug}. The \textit{throughput} relationship is explained in Section \ref{sec:corr:TP} while the \textit{churn} relationship were explained in Section \ref{sec:corr:lt}.   Team ten has a significant positive correlation for \textit{throughput}, \textit{throughput bug}, \textit{bugs finished, quarter} and \textit{lead time}. The \textit{throughput} relationship were stated in Section \ref{sec:corr:WIP}. Table \ref{DS:corr:Bugs:v2} shows a medium or strong relationship for \textit{throughput}, \textit{throughput bug}, \textit{bugs finished, quarter}, \textit{avg days backlog, days}, \textit{lead time} and \textit{churn bug}. 
...
\end{tabular}
 ...
 The Table \ref{corr:TP:V2} shows that team one has a significant positive correlation for \textit{throughput feature}. The reason \textit{throughput} relation were stated in Section \ref{sec:corr:WIP}.Team two has a significant positive correlation for \textit{WIP}, both \textit{throughputs} sub variable, \textit{bugs}, \textit{bugs finished, quarter} and \textit{lead time}. Team three has a positive correlation for \textit{WIP}, both \textit{throughputs} sub variables and \textit{bugs}. Team four has a significant relationship for \textit{WIP}, both \textit{throughputs} variable, \textit{bugs finished, quarter}, \textit{lead time}, \textit{churn} and \textit{churn feature}. The \textit{churn} relationship were described in Section \ref{sec:corr:lt}. For team five has \textit{throughput bug}, \textit{bugs}, \textit{bugs finished, quarter}, \textit{avg, days in backlog, bugs}, \textit{lead time}, \textit{churn} and \textit{churn feature}. The \textit{churn} relationship is explained in Section \ref{sec:corr:churn}. The \textit{throughput} relationship were described in \ref{sec:corr:WIP}. 
 \begin{table}[!htbp]
  \begin{adjustwidth}{-2.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
\end{table}
 ...
 Team six has a significant positive correlation for \textit{throughput feature} and \textit{lead time}. The \textit{throughput} relationship were stated in Section \ref{sec:corr:WIP}.  Team seven has a significant positive correlation value for \textit{throughput feature}. The relationship were explained  \ref{sec:corr:WIP}. Team eight has a significant positive correlation value for \textit{throughput bugs}, \textit{bugs}, \textit{bugs finished, quarter}, \textit{churn} and \textit{churn bug}. The \textit{throughput} relationship were stated in Section \ref{sec:corr:bug}. The \textit{churn} relationship is explained in Section \ref{sec:corr:lt}. The conclusion from that relationship stated that \textit{churn feature} represents most of the \textit{churn} variable.  The values from Table \ref{corr:TP:V2} and \textit{churn} correlation Table \ref{DS:corr:churn:v2} proves the first assumption from Section \ref{sec:corr:lt}, which states that both the \textit{churn} sub variables contributes to \textit{churn}. %Since there are two tables showing different values against each other. 
...
\textit{Churn feature} for team seven has the correlation value of .91 while \textit{churn bug} has the correlation value 0.13. The descriptive statistic Tables \ref{DS:Churn:7}, \ref{DS:CF:7} and \ref{DS:CB:7} shows that \textit{churn} contains 359 dates and has a total mean of 77.3.  \textit{Churn feature} represents 141 of these dates, and has a total mean of 121.2 .  \textit{Churn bug} represents the 218 remaining dates and has a total mean of 48.9. These values, one could assume that both  \textit{churn feature} and  \textit{churn bug} contribute to  \textit{churn}, but the churn correlation table disapproves that. Which once again shows that the descriptive statistic tables can be used as indication of relationship, but can't prove them. The Figure \ref{fig:a:7} shows clearly the significant positive relationship between \textit{churn} and \textit{churn feature}.
...
The std.dev for both the results are 0.4, while the median is 0.1 when team size is \textbf{not} taken into account and -0.2 for the other case. The std.dev empower the assumption of a relationship between \textit{churn} and \textit{WIP}, since the values are spread and their is no pattern.  
