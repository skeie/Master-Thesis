...
\section{Team size}
As the two different results sections in Chapter \ref{ch:res} shows, it's important to take team size into account when looking for relationship between variables such as \textit{WIP}, \textit{throughput} and \textit{bugs}. The Table \ref{DS:corr:TS} shows the overall correlation value for \textit{WIP} is 0.6, for \textit{throughput} is 0.5 and for \textit{bugs} is 0.4. The standard deviation shows a value of 0.2 for \textit{WIP}, 0.3 for \textit{throughput}  and for \textit{bugs} it's 0.2. The overall correlation values shows the positive correlation, and what impact \textit{team size} has on the variables. The standard deviation shows a small gap between these three correlation variables and \textit{team size}, which means that each of the teams' values are relative alike.


\section{WIP and lead time}
As stated in Section \ref{sec:in:lt}, lead time could be important to measure productivity. Each development process would like to get their lead time as low as possible. In Section \ref{WIPsec}, there is stated that WIP-limits are important to reduce lead times. The result from when team size was not taken into account shows that when WIP increase, so does lead time. Team one, four, five, seven and ten has significant positive correlation between WIP and lead time and both team two and three has a medium correlation as shown in Table \ref{DS:corr:WIP}. The overall correlation value is 0.5, as shown in Table \ref{DS:corr:WIP}.

The result from when team size was taken into account shows a weak linear relationship. Three of the ten teams has a significant positive correlation. While the rest of teams correlation values varies from -0.28 to 0.32. The standard deviation for \textit{lead time} is 0.4 as shown in Table \ref{DS:corr:WIP:v2}. The standard deviation shows a high proliferation of values, which shows evidence of spread relationship between \textit{WIP} and \textit{lead time} across the teams.

Despite \textit{WIP} and \textit{lead time} don't has a close relationship,  \textit{Lead time} has a close positive relationship to four of the five individual variables, as shown in table \ref{DS:corr:LT:v2}. \textit{Throughput},\textit{Bugs} and  \textit{Bugs finished, quarter}  has the total correlation value 0.5, while \textit{churn} has the value of 0.4. This shows that \textit{lead time} is one of most dependent variables in SI's software development process. 

\section{WIP and bugs}
To reduce bugs should be a goal independent of software process and methods. If WIP-limit could reduce bugs and still produce an acceptable level of throughout, it would matter in software development. In the article by Shinkle \parencite{Shinkle},  when WIP was to high, the number of bugs increased. When the team size is not taken into account,  four of the teams from SI has a significant positive correlation value between \textit{bugs} and \textit{WIP}, all ten of them had a positive correlation value shown in table \ref{corr:TP}. The Table \ref{DS:corr:TP} shows that the overall correlation value for SI between  \textit{bugs} and \textit{WIP} is 0.4, which is a medium strong correlation. This values shows that, if WIP decreases, so would bugs.

The result when team size is taken into account, shows that one team has a significant positive correlation relationship. But, nine of of ten teams had a positive correlation shown in Table \ref{corr:WIP:V2}, although most of them had a weak linear relationship. The variables \textit{bugs finished, quarter} and \textit{avg days backlog, bugs} is used as a side measure to measure the impact of the different variables and bugs. 

When team size is \textbf{not} taken into account \textit{bugs finished, quarter}, the overall correlation is 0.3. When team size is taken into account, the overall correlation value is 0.1 For \textit{avg days backlog, bugs} the overall correlation value when team size is \textbf{not} taken into account is 0.1 and the value when the team size is taken into account is 0. This shows that Both \textit{bugs finished, quarter} and \textit{avg days backlog, bugs} has no linear relationship to \textit{WIP} when team size is taken into account, and a small linear relationship when team size is  \textbf{not} taken into account. Either way, this shows that WIP variable impact on bugs is low.   

But SI started to use WIP-limits when implementing Kanban, so it's hard to find any evidence between \textit{WIP} and \textit{Bugs} to see if was to high. 

\section{WIP and throughput} 
In this thesis, throughput is a measure to see how productive teams are. In Secion \ref{WIPsec}, there was stated by various people that when lowering WIP-limit, throughput increases. When team size is \textbf{not} taken into account,  All of teams have a positive correlation between \textit{WIP} and \textit{throughput}, and seven of the teams have a significant correlation value, as shown in table \ref{corr:TP}. Based on these variables, SI should increase or remove their WIP-limit, and then the throughput will increase as well. When taken time size into account, the Table \ref{DS:corr:WIP:v2}  shows that when \textit{WIP} increases so does \textit{throughput} for nine out of ten teams. Team ten is the only team with a negative correlation.  Team, five, six, eight and ten has low small correlation between \textit{throughput} and \textit{WIP}, Team one and seven has a low-medium relationship, team two, three, four and nine has a significant linear relationship. The relationship between \textit{WIP} and \textit{throughput} when team size is \textbf{not} taken into account shows a overall correlation value of 0.6, shown by table \ref{DS:corr:WIP}. When time size is taken into account, the correlation table shows a overall correlation value of 0.4, shown by \ref{DS:corr:WIP:v2}. This shows that when team size is omitted, \textit{WIP} and \textit{throughput} has a medium-strong relationship, otherwise a strong relationship.   

The medium-strong relationship means that when \textit{WIP} decreases, \textit{throughput} decreases as well to an extent. The throughput correlation Table  \ref{DS:corr:TP:v2} shows \textit{Bugs} has the overall correlation value of 0.6 and \textit{lead time} has the overall value of 0.5. One could see that if \textit{throughput} increases, so does both \textit{lead time} and \textit{bugs}. There is one factor one must taken into account when determine the relationship between \textit{throughput} and \textit{bugs}. When throughput is high, one produces more. When one produces more, there is normal to produce more bugs as well. One can see in Table \ref{DS:corr:TP:v2} that SI overall produce more \textit{throughput bug}, than \textit{throughput feature}. Which empower the assumption that SI produces more bugs when \textit{throughput} decreases. The overall correlation value of \textit{Bugs finished, quarter} is 0.3. This means that there is a weak positive correlation, which states that even through more bugs are produced, bugs are generally finished within the same month.  The overall value of \textit{Avg days backlog, bugs} is 0.1. 0.1 represents a no linear relationship.

The \textit{throughput's} sub variables both have a overall correlation of 0.5 when team size is \textbf{not} taken into account and 0.3 when time size is taken into account. This data shows that both \textit{throughput's} sub variables have the same relationship to \textit{WIP}. If for some reason \textit{throughput feature} had a overall value of 0.6 and \textit{throughput bug} had a overall value of 0.1. This values would stated that when WIP decreases, SI would produce more feature tasks than bug tasks.    
...
\chapter{Conclusion}
\label{ch:conc}
In this thesis the research question "Does WIP limit in software development matter?". 
Isolated set, if WIP increase, so does throughput. Which is good, but of one take take all the factors into account, one will see that lead time increases as well. 

Have to take team size into account when discussing WIP - limit's impact in software development. 

If WIP - limit matters in software development should it reduce bugs.
...
The article "Studying Lean-Kanban Approach Using Software Process Simulation" \parencite{DavidAnderson} simulated a process a lean-kanban approach as stated in \ref{WIPsec}. The approach showed a software process with a backlog of 100 tasks and 40 added through the development. The simulation showed when WIP was limit, the software process used 100 days and 120 otherwise. 
