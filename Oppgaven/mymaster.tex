...
\textbf{Results}: Some of the result from this work showed a mean correlation  of 0.4 between \textit{WIP} and \textit{throughput}, 0.2 between \textit{WIP} and \textit{bugs} and \textit{lead time} and -0.1 between \textit{WIP} and  \textit{churn} across the teams.\\ 
\textbf{Conclusion}:\\
...
In this chapter there will be a brief introduction to software development process/methods as Waterfall (Section \ref{sec:WP}), Scrum (Section \ref{sec:Scrum}), Lean (Section \ref{sec:Lean}), Kanban (Section \ref{sec:Kan}) with affiliated tools.
\section {Waterfall}
\label{sec:WP}
...
The reason SI and the data set from SI is analyzed in this work is because a prior research used the data set \parencite{Dag}. Since Sj\o berg is the supervisor of this work and had access to the data set, so it was convenient to use the same data set.
...
The data from SI was analyzed on team level using the developed software program and SPSS. The software program computed the variables shown in Table \ref{des} for all of the teams.
\newpage
\begin{table}[htbp]
\begin{center}
...
\section{SPSS}
\label{sec:SPSS}
"IBM\circledR  SPSS\circledR Statistics is a comprehensive system for analyzing data. SPSS Statistics can take data from almost any type of a file and use them to generate tabulated reports, charts and plots of distributions and trends, descriptive statistics, and complex statistical analyses." \parencite{IBM}. SPSS will be used to analyze the derived data from the developed software program by using two statistics method; correlation and case summaries. 


\section{The workflow}
\label{sec:workflow}
The workflow of creating the result showed in Chapter \ref{ch:res} is showed in Figure \ref{dataflow}. First is the data converted from Microsoft TFS server to an excel document. The excel document is converted to a .csv file which the software program measure and produce documents containing WIP, throughput, lead time etc. The documents are used by SPSS to compute correlation tables and descriptive statistics.  
\begin{figure}[h] 
  \center
...
\subsection{Step 1: Gather all dates into a Arraylist}
\label{sub:stepOne}
...
\subsection{Example of step 1, 2 and 3}
\label{sec:Example}
This section will provide a comprehensive example of how the WIP algorithm works.  
Figure \ref{wip_timeline}  shows task ids on the y-axis and dates on the x-axis. The green line indicates the duration of the task. The figure helps  visualize how many WIPs there are in progress for a given date. For example on the date 2010-10-12, tasks 3, 5 and 6 are in progress, which means the WIP is 3 for 2010-10-12. The dates from Table \ref{wt:2}  will be used to illustrate how the algorithm measures WIP. The Section \ref{sub:sub:st1} explains how the program gather all dates into a Arraylist, Section \ref{sub:sub:st} explains how the program measure the remaining  dates and Section \ref{sub:sub:st3} explains how the WIP per day is calculated.
...
\subsubsection{Step 1: Gather all dates into a Arraylist}
\label{sub:sub:st1}
The program will first read in line 1 with task id one, showed in Table \ref{wt:2}.  The program creates the WIP-object for line 1, shown by Listing \ref{lst:WIPOBJ}. After the WIP-object is created it is put in team one's Arraylist. The program will follow the exact same procedure until all the dates are read. 
...
\subsection{Churn}
\label{sec:churn}
As stated in Section \ref{sec:Churn}, in order to take churn into account one has to know its good surrogates. SI has gathered churn with help of TFS \parencite{Dag}. The TFS system automatically records data such as churn and lead time. Based on TFS one can know that churn for SI is a good surrogate.
...
The moderating variables, bugs finished, quarter and mean days backlog, bug is used as help variables. A correlation tables for these variables will not be provided in Chapter \ref{ch:res}. 
