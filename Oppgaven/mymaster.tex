...
\chapter{Results}                     
\label{ch:res}
The result chapter is split into two sections. The first section is team size is a part of the results. The second section is each variable divide by  their teams team size.  Each of the sections are presented with five correlation tables and five corresponding descriptive statistics tables.
...
As shown by Table \ref{DS:corr:TS}, most the data has a close relationship to team size, which is makes sense. To try to remove bias, each of eleven variables were divided by their team size. The mean value for each variables per quarter were divided by the time size of the same quarter.
...
\subsection{Bugs - correlation - divide per time size}
\FloatBarrier
\begin{table}[H]
  \begin{adjustwidth}{-2.5cm}{}
 \centering
...
\subsection{Lead time - correlation - divide per time size}
\begin{table}[H]
  \begin{adjustwidth}{-2.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
\subsection{WIP - correlation - divide per time size}
\begin{table}[H]
  \begin{adjustwidth}{-1.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
The result from when team size was taken into account shows a weaker linear relationship. The correlation value is 0.2. Three of the ten teams has a significant positive correlation. While the rest of teams correlation values varies from -0.28 to 0.32, as shown by Table \ref{DS:corr:WIP:v2}. The standard deviation for \textit{lead time} is 0.4 as shown in Table \ref{DS:corr:WIP:v2}. The standard deviation shows a high proliferation of values, which shows evidence of spread relationship between \textit{WIP} and \textit{lead time} across the teams.
...
Churn is used as surrogates for effort in this thesis. It turns out based on the results, independently of time size, there is no relationship between \textit{WIP} and \textit{churn} or the sub variables of churn. Both the results shows respectively a overall correlation of -0.1 in Table \ref{DS:corr:WIP:v2} and 0 in Table \ref{DS:corr:WIP}. When time size is \textbf{not} taken into account \textit{Churn} has no linear relationship between any of the other variables, if one disregards \textit{churn's} sub variable shown in Table \ref{DS:corr:Churn}. When time size is taken into account, \textit{bugs finished, quarter} has a strong relationship with a value of 0.6, while \textit{lead time} has a value of 0.4 which is a medium-strong relationship, shown in Table \ref{DS:corr:churn:v2}.

\section{Team size}
As the two different results sections in Chapter \ref{ch:res} and the sections above shows, it's important to take team size into account when looking for relationship between variables such as \textit{WIP}, \textit{throughput}, \textit{lead time} and \textit{bugs}. If team size were left out as a variable in this case study, \textit{WIP} would had a linear relationship of 0.6 with \textit{throughput}, 0,4 with \textit{bugs} and 0.4 with \textit{lead time}. The three correlation values increased by 0.2 compared to when time size was taken into account. These variables would have showed a stronger results towards the fact that WIP-limit matter in software development. 

\section{WIP}
The article "Studying Lean-Kanban Approach Using Software Process Simulation" \parencite{DavidAnderson} simulated a  lean-kanban process as stated in \ref{WIPsec}. The simulation showed when WIP was limit, the software process used 100 days and 120 otherwise. The article "Simulation of software maintenance process, with and without a work-in-process limit" \parencite{SMR:SMR1599} showed that WIP-limit can lead to improvement in throughput. The article also showed that when taking a process without WIP-limit, and simulate it using WIP-limit outperformed the original process. independently of team size, this case study shows another result. This case shows when WIP is increased, so is throughput. Which is opposite of what the people states and studies shows. 
...
If WIP limit matters in software development, it should reduce bugs, increase throughput, decrease lead time ect. Based on the results from the case study, \textit{WIP} has an impact on \textit{throughput}. The results shows a positive linear relationship between WIP and throughput. The previous research has stated to lower WIP to decrease throughput. This research shows otherwise. Based on the results, throughput decreases when WIP decreases. 

If \textbf{not} taken team size into account, one can say that \textit{WIP}-limits has a greater impact on software development 

%skrive her om de to andre research questions

However, the results from this paper shows some other findings. The importance of taking team size into account when measuring a similar research. The churn variable has no or little impact on the software process. The results shows that churn has no linear relationship between 

limit matteres  Isolated set, if WIP increase, so does throughput. Which is good, but of one take take all the factors into account, one will see that lead time increases as well. 

Have to take team size into account when discussing WIP - limit's impact in software development. 


%TP
SI should increase or remove their WIP-limit, and then the throughput will increase as well. 
If the team size were not taken into account, four of the teams would have a strong linear relationship with \textit{WIP}. Another four teams had a moderate linear relationship, one would have a weak relationship and one team would have no linear relationship. 

It looks like churn measure is irrelevant for this study. 




\section{Future work}
The conclusion from this thesis is made on one case study. I would recommend to do the same calculation as in this thesis with another data set and compare the outcome. I would also suggest an different approach as looking more deeply into the relationship between \textit{WIP} and  \textit{team size}. In this thesis the mean number of employees per quarter's number used for measurements. I suggest one measure the number of employees working on each task and take the mean value for each quarter and measure it up agains the value in this thesis. 

I will also suggest to to look at the values up against release dates. The number of WIP's, throughput, bugs et cetera usually decrease around release date,  which could cause bias in the data.

The data in this thesis shows the importance of taken time size into account when measuring the importance of principle. 
Would suggest to do an experiment with WIP - limit for SI, where WIP-limit is systematically decreased to see   

Take the other study by SI and investegate the results
