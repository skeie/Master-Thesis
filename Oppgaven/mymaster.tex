...
\label{chap:RM}
In this chapter the research methods will be revealed and the reason why the data from Software Innovation is picked for this master thesis.  The first section \ref{sec:coc} will be about the choice of case and complementary information about SI. The next section \ref{WPD} reveals the algorithm of how the program measure WIP for each day. The next section \ref{sec:Example} will provide a comprehensive example of how the program measure WIP per day. The consecutively sections reveals the algorithm of how the program measure bugs (section \ref{sec:bug}) throughput (section \ref{sec:TP}),  churn (section \ref{sec:churn}) and lead time (section \ref{sec:LT}) per day.  In the last section information about SPSS will be given (section \ref{sec:SPSS})
...
In order to try answering the research questions stated in chapter \ref{chap:RQ} the data from SI will be interpreted. The data set contains thirty columns with different data for each task, mostly of this columns are irrelevant for this study but the important columns is stated in table \ref{IC}. The reason SI and the data set from SI is analyzed in this thesis is based on the article "Quantifying the Effect of Using Kanban versus Scrum" by Dag Sjøberg, Anders Johnsen and Jørgen Solberg \parencite{Dag} since Dag is the supervisor of this thesis and had access to the data set. %skrive om siste settning?

%ha subsecton{SI} her?From 2001 to 2006 SI used the Waterfall process with a life cycle of design, implementation, testing, and deployment for each new release. In 2007 SI changed to Scrum. Scrum was implemented with the standard elements of Scrum: \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]\item Cross functional teams\item Sprint planning meetings \item Estimation of work items using planning poker\item Daily standup meetings\item Sprints of three weeks\end{itemize}After a three week sprint a fully tested shippable code was ready. In 2010 SI went from Scrum to Kanban. SI felt that Scrum was too rigid and didn't fit their purpose, they also feared that inaccurate estimation and time boxes gave them longer lead times. They also saw Scrum planning meetings as waste which reduced productivity and quality\parencite{Dag}. SI decided to implemented Kanban in the following manner. When a work item starts SI tries to make the item flow through all the stages until it's ready for release. This procedure happens as quick as possible. In order for an item to be ready for release it has to be at a satisfactory quality level, which is defined by SI. SI also implemented WIP limits, if the WIP limit is reached, no new tasks are started until another task is finished also known as just-in-time. \parencite{Dag}.In this thesis the main goal is to see if WIP matters in agile method. It will be done by interpret the data set from SI on team level.  SI has about 25 teams. From the data set there are six teams who have worked on the most tasks. These six teams will be used in this thesis in order to try answer the research question in section \ref{chap:RQ}. The data from SI will be analyzed using a program made for this thesis, which will extract the variables shown in table \ref{table:variable} for each of the six team. This data will be further analyzed using SPSS. With SPSS each variable will be gathered based on which quarter of the year it belongs to and team. The analyzed data will be presented in chapter \ref{chap:res}. 
\newpage
The following tables (table \ref{table:variable} and table \ref{dataset}) shows the five variables that will measured by the program and the latter shows excerpt from the data set
\begin{table}[!ht]
\begin{center}
\begin{itemize}
\item WIP 
...
In the two following subsection the important columns from the data set will be shown and a brief description will be given (subsection \ref{sub:sub:tc}) and the next subsection contains a table of which columns needed to calculate the variable stated in table \ref{table:variable} In the following table (table \ref{tab:measurementDone}) information about day, date and quarter is stated.   
\begin{table}[!ht]
\centering
\begin{itemize}
...
\subsection {Columns needed for calculating the variables per day}
\begin{table}[!ht]
\begin{center}
...
To measure churn the data set from SI contains three columns as shown in table \ref{table:churn}. To complete the measure the three columns are multiplied.  For task id one, the churn is 2028 ($352+307+1369 = 2028$). The task with id six has zero churn. In the data set there exists tasks with zero churn, these tasks don't need code in order to be completed such tasks needs support to be finished.
...
\section{Lead time}
\label{sec:LT}
The program does not need to analyze  the lead time for each task. The lead time for each task is stated in the set as shown in table \ref{table:LT}. The program will gather all the tasks that are started on the same day and belong to the same team and add up their lead time together as shown in code listing \ref{LTcode}.   
\begin{table}[!ht]
\begin{center}
\begin{tabular}{ | l | l | l | }
\hline
