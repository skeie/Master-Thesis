...
\item If so, how can one find the optimal WIP?
...
\label{sec:corr:WIP}
Table \ref{corr:WIP} shows the correlation table for \textit{WIP}. The variables are shown vertically in the correlation table. Horizontally are the corresponding teams. The team names are shortened. Team one is shortened to T1, team two is shortened to T2 and so on.
The correlation table shows team one, three, four, six, seven, nine and ten has significant positive relationship between \textit{WIP} and \textit{throughput}. \textit{Throughput feature} and \textit{throughput bug} is subset of \textit{throughput}. It's natural that these sub variables have a significant positive correlation to \textit{WIP}, when \textit{throughput} has. But, for team one, six, seven and ten that's not the case. 
...
\textit{Throughput feature} has a significant correlation for team eight as well. The \textit{throughput} relationship is explained in Section \ref{sec:corr:bug}.
Team three and seven has a significant positive correlation between \textit{lead time} and \textit{bugs}. Team seven also has a significant positive correlation between \textit{lead time} and \textit{bugs finished, quarter}. Team two has a significant positive correlation for \textit{throughput} and \textit{throughput bug}. The \textit{throughput} relationship is explained in Section \ref{sec:corr:lt}. The descriptive statistic table for lead time shown in Table \ref{DS:corr:LT} shows that both \textit{WIP} and \textit{throughput} has a medium to strong correlation with \textit{lead time}. 
\begin{table}[H]
 \centering
 \begin{tabular}{ | l | r | r | r | r | r | r | }
 \hline
...
\label{sec:corr:bug}
This section contains information about the correlation table between the variables and \textit{bugs}. In Table \ref{corr:bug} Teams one, two, three, four, five, nine and ten all have a significant positive correlation between \textit{bugs} and \textit{throughput}. For team three, four, five and nine's case, all \textit{throughput} variables have a significant positive correlation with \textit{bugs}. For team one and ten, the \textit{throughput} variables differ. The \textit{throughput} relationships were stated in Section \ref{sec:corr:WIP}. 
 
Team one, three, four and nine has a significant positive correlation for \textit{WIP}.  Team one also has a significant positive correlation for \textit{lead time}, \textit{churn}, \textit{churn feature} and \textit{team size}. The \textit{churn} relationship for team one were stated in Section \ref{sec:corr:WIP}. Team five has a significant positive correlation for \textit{team size} while \textit{team eight} has a significant positive correlation for \textit{churn bug}. The \textit{churn} relationship were explained in Section \ref{sec:corr:lt}.
\begin{table}[h]
 \begin{adjustwidth}{-2.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
Team two's \textit{throughput feature} differ from \textit{throughput} based on the correlation values form Table \ref{corr:bug}. One could believe the reason is because \textit{throughput bug} consists of 2/3 of \textit{throughput} (460/690) dates as shown in the descriptive statistic Tables \ref{DS:Throughput:2} and \ref{DS:TPB:2}. But the two tables and Table \ref{DS:TPFT:2} shows that the total mean of \textit{throughput} is 4.39. While for \textit{throughput bug} it's 4.8 and 3.7 for \textit{throughput feature}. These three variables are quiet close, which could reflect that these three variables could be close, based on correlation measurement. 
But, the Figures in \ref{corr:Difference:2} and the throughput correlation table in \ref{sec:corr:TP} shows that they are not. The Figures \ref{fig:b:2} shows a significant positive correlation, while Figure \ref{fig:a:2} shows dots that are more randomly placed. The throughput correlation Table \ref{corr:TP} shows a correlation relationship with a value of 0.97 for \textit{throughput bug} and  0.1 for \textit{throughput feature}.
\begin{figure}[h] 
  \begin{subfigure}[b]{0.5\textwidth}
  \center
...
\end{figure}
\newpage
Team six, seven and nine has a significant positive correlation between the variable \textit{Bugs finished, quarter} and \textit{lead time}. Team nine also has a significant negative correlation between \textit{bug} and \textit{churn feature}, but not \textit{churn}. \textit{Churn feature} represents 201 of 538 dates as shown in Tables \ref{DS:Churn:9} and \ref{DS:CF:9}.  The total mean of \textit{churn feature} is 115.9, the total mean for \textit{churn} is 72.2 and the total mean for \textit{churn bug} is 46.1, as shown in the previous stated tables and Table \ref{DS:CB:10}. Judging from these numbers, both \textit{churn feature} and \textit{churn bug} contribute to \textit{churn}. The Figure \ref{corr:Difference:9} shows the correlation values in a graph. There is hard to find any pattern or evidence that \textit{churn feature} and \textit{churn bug} contribute to \textit{churn}. But, The churn correlation Table \ref{corr:churn} shows \textit{churn feature} correlation values is 0.60 and \textit{churn bug} correlation value is 0.39. This proves that both the sub variables of \textit{churn} contribute, with \textit{churn feature} contributing slightly more . 
...
Team three and seven has a significant positive correlation between \textit{bugs} and \textit{lead time}. Team six has a significant positive correlation for \textit{throughput bug}. The \textit{throughput} relationship were explained in Section \ref{sec:corr:WIP}. Team eight has a significant positive correlation for \textit{throughput bug}. The \textit{throughput} relationship is explained in Section \ref{sec:corr:TP}.
In Table \ref{DS:corr:Bugs}, one can see that \textit{WIP} with a mean value of 0.4 has a medium correlation with \textit{bugs} as stated in Section \ref{corr:WIP}.  \textit{Throughput}  has a value of 0.6. This shows that \textit{throughput} have a high correlation with bugs.
...
Team eight also has a significant positive correlation between \textit{throughput} and \textit{churn}, but the sub variables don't has a significant relationship. The \textit{churn} relationship were stated in Section \ref{sec:corr:lt}. Team four's \textit{churn} variables also differ. The reason for this were stated in section \ref{sec:corr:lt}. Team one, two and four has a significant relationship between \textit{throughput} and \textit{lead time}. According to Table \ref{DS:corr:TP}, \textit{bugs} have a strong positive correlation with \textit{throughput} as stated Section \ref{sec:corr:bug}. \textit{WIP} has a strong positive relationship with \textit{throughput} which where stated in Section \ref{sec:corr:WIP}. \textit{Team size} and \textit{lead time} has a medium to strong relationship to \textit{throughput}. The sub variables \textit{throughput feature} (0.8) and \textit{throughput bug} (0.7) has  a strong and \textit{throughput}. 
...
\end{tabular}
 ...
 The Table \ref{corr:TP:V2} shows that team one has a significant positive correlation for \textit{throughput feature}, but not \textit{throughput}. The reason \textit{throughput} relationship were stated in Section \ref{sec:corr:WIP}.Team two has a significant positive correlation for \textit{WIP}, both \textit{throughputs} sub variable, \textit{bugs}, \textit{bugs finished, quarter} and \textit{lead time}. Team three has a positive correlation for \textit{WIP}, both \textit{throughputs} sub variables and \textit{bugs}. Team four has a significant relationship for \textit{WIP}, both \textit{throughputs} variable, \textit{bugs finished, quarter}, \textit{lead time}, \textit{churn} and \textit{churn feature}, but not \textit{churn bug}. The \textit{churn} relationship were described in Section \ref{sec:corr:lt}. For team five has \textit{throughput bug}, \textit{bugs}, \textit{bugs finished, quarter}, \textit{avg, days in backlog, bugs}, \textit{lead time}, \textit{churn} and \textit{churn feature} a positive correlation, but not \textit{churn bug}. The \textit{churn} relationship is explained in Section \ref{sec:corr:churn}. The \textit{throughput} relationship were described in \ref{sec:corr:WIP}. 
 \begin{table}[!htbp]
  \begin{adjustwidth}{-1.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
\end{table}
 ...
 Team six has a significant positive correlation for \textit{throughput feature} and \textit{lead time}. The \textit{throughput} relationship were stated in Section \ref{sec:corr:WIP}.  Team seven has a significant positive correlation value for \textit{throughput feature}. The relationship for \textit{throughput} were explained  \ref{sec:corr:WIP}. Team eight has a significant positive correlation value for \textit{throughput bugs}, \textit{bugs}, \textit{bugs finished, quarter}, \textit{churn} and \textit{churn bug}. The \textit{throughput} relationship were stated in Section \ref{sec:corr:bug}. The \textit{churn} relationship is explained in Section \ref{sec:corr:lt}, the \textit{churn} relationship for team eight is another example correlation and transitive. 
 Team nine has a significant correlation value for \textit{WIP}, both sub variables of \textit{throughput} and \textit{bugs}. Team ten has a significant positive correlation for \textit{throughput bug}, \textit{bugs}, \textit{bugs finished, quarter} and \textit{lead time}. The \textit{throughput} relationship were explained in Section \ref{sec:corr:WIP}. Based on Table \ref{DS:corr:TP:v2}, \textit{throughput} has either medium or a strong relationship between \textit{WIP}, both \textit{throughput} sub variables, \textit{bugs}, \textit{bugs finished, quarter} and \textit{lead time}.
...
Team one has a significant positive correlation for \textit{bugs finished, quarter}, \textit{lead time} and \textit{churn bug}, but not \textit{churn}. The \textit{churn} relationship were stated in Section \ref{sec:corr:WIP}.  Team two has a significant positive correlation for \textit{throughput feature}, \textit{bugs finished, quarter} and both \textit{churn} sub variables. The \textit{throughput} relation were stated in Section \ref{sec:corr:bug}. Team two also has a significant negative correlation for \textit{avg days in backlog, bugs}. Team three has significant positive correlation values for \textit{bugs}, \textit{bugs finished, quarter} and both \textit{churn} sub variables.  Team four has a significant positive correlation for \textit{WIP}, all \textit{throughput} variables, \textit{lead time} and \textit{churn feature}. The \textit{churn} relationship were stated in Section \ref{sec:corr:lt}. Team five has a significant correlation value for \textit{throughput}, \textit{throughput bug}, \textit{bugs}, \textit{bugs finished, quarter}, \textit{avg days in backlog, bugs}, \textit{lead time} and \textit{churn bug}. The \textit{throughput} relationship were stated in Section \ref{sec:corr:WIP}. 
 \begin{table}[H]
\begin{adjustwidth}{-2.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
\end{adjustwidth}
\end{table}
Team six has a significant positive correlation of \textit{avg days in backlog, bugs} and \textit{churn feature}, The \textit{churn} relationship were explained in Section \ref{sec:corr:WIP}. Team seven has a significant positive correlation to \textit{churn feature}. The \textit{churn} relationship were stated above in this section. Team eight has a significant positive correlation for \textit{throughput}, \textit{bugs finished, quarter}, \textit{lead time} and \textit{churn feature}. The \textit{churn} relationship were described in Section \ref{sec:corr:lt}, while the \textit{throughput} relations were described in Section \ref{sec:corr:bug}. Team nine has a significant positive correlation for both the \textit{churn} sub variables. Team ten's has a significant positive correlation for \textit{avg days in backlog, bugs} and \textit{churn bug}. The \textit{churn} relationship were explained above in this section.  The Table \ref{DS:corr:churn:v2} shows medium or strong overall linear relationship for  \textit{throughput}, \textit{throughput bug}, \textit{bugs}, \textit{bugs finished, quarter}, \textit{lead time} and both \textit{churn} variables. 
...
\end{table}
\newpage
Team one and five has a significant positive correlation between \textit{bugs} and \textit{team size}. Team two has a significant positive correlation between \textit{avg days in backlog, bugs} and \textit{team size}. Team one also has a positive correlation between \textit{team size} and \textit{lead time},  as well as \textit{churn feature}. The \textit{churn} relationship were stated in Section \ref{sec:corr:WIP}. 
...
\chapter{Discussion}
\label{ch:dis}
In this chapter a discussion based on the research questions, the different results from this study and what literature says are presented. 
...
To minimize bugs should be a goal independent of software process and methods. If WIP-limit could reduce bugs without compromising, one could conclude that WIP-limit matter in software development. In the article by Shinkle \parencite{Shinkle}, when WIP was too high, the number of bugs increased. 
...
When team size is taken into account, the average correlation value is 0.2, shown in Table \ref{DS:corr:WIP:v2}.  Eight out of ten teams have a positive correlation relationship, but only one team has a significant positive value. The  \textit{bug finished quarter} and \textit{avg days backlog, bugs}  has the average correlation values 0.2 and -0.1. 
...
The standard deviation for both the results are 0.4, while the median is 0.1 when team size is \textbf{not} taken into account and -0.1 for the other case. The standard deviation empower the assumption of a relationship between \textit{churn} and \textit{WIP}, since the values are spread and their is no pattern.  The Table \ref{corr:All} shows a overall correlation value of 0.06 when team size is taken into account. The Table \ref{corr:All:V2} shows a correlation value of -0.07. These values also shows that there is no linear relationship between \textit{churn} and \textit{WIP}.


In Section \ref{sec:corr:churn}, when team size is \textbf{not} taken into account \textit{Churn} has no linear relationship between any of the other variables, if one disregards \textit{churn's} sub variable shown in Table \ref{DS:corr:Churn}. When  team size is taken into account, \textit{bugs finished, quarter} has a strong average relationship with a value of 0.6, while \textit{lead time} has a value of 0.4 which is a medium-strong relationship, shown in Table \ref{DS:corr:churn:v2}. Based on Table \ref{corr:All}, \textit{churn} has a significant positive correlation value to both the sub variables and a significant negative correlation to \textit{team size}. When team size is taken into account, \textit{churn} has a significant positive correlation to \textit{throughput}, \textit{throughput bug}, \textit{bugs}, \textit{bugs finished, quarter}, \textit{avg days in backlog, bugs}, \textit{lead time} and \textit{churn bug}. 



\section{Team size}
As the different results sections in Chapter \ref{ch:res} and the sections above states, it's important to take team size into account when looking for relationship between variables such as \textit{WIP}, \textit{throughput}, \textit{lead time} and \textit{bugs}. If team size were left out as a variable in this case study, \textit{WIP} would have a  average linear relationship of 0.6 with \textit{throughput}, 0.4 with \textit{bugs} and 0.4 with \textit{lead time}. These three correlation values increased by 0.2 compared to when  team size was taken into account. The two overall correlation Tables  \ref{corr:All} and \ref{corr:All:V2} shows that seven of the variables has a significant positive correlation with \textit{team size}. The table also shows eight out of eleven variables has a significant correlation value with \textit{WIP}, while the Table \ref{corr:All:V2} shows one out of ten variables has a significant relationship, when team size is taken into account. If team size was not taken into account these variables would have showed stronger results towards the fact that WIP-limit matter in software development.
