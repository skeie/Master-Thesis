...
In this chapter the results are presented. The results are presented with five correlation tables and five corresponding descriptive statistics tables. Vertically in the correlation tables, the correlation variables are stated. Horizontally is the corresponding teams are stated. The team names are shortened in order to fit the tables. Team one is shortened to T1, team two is shortened to T2 and so on. The correlation tables is based on the mean value from the variables. Some of the variables from the correlation tables are shown in Table \ref{des} and are explained in chapter \ref{ch:DCC}, the rest will be explained here. Variables with suffix of "Feature" means tasks labeled with type "feature" are the only one who is counted. The same goes for variables with suffix "Bug". The "Bugs finished, quarter" variable represents how many tasks labeled "Bug" that are finished within the same quarter as it was created.  The "Avg days backlog, bug" variable is used to see the average number of days bugs are in backlog before pulled out. These two variables are used to see the prioritizing of bugs within a quarter as well as indicator of the workload together with churn for a quarter.   


This chapter will be presented in the following manner. The first, Section \ref{sec:corr:WIP} shows the correlation table for WIP and the corresponding descriptive statistics table, Section \ref{sec:corr:lt} shows the correlation table for leadtime  and the corresponding descriptive statistics table, Section \ref{sec:corr:bug} shows the correlation table for bug and the corresponding descriptive statistics table, Section \ref {sec:corr:TP} shows the correlation table for throughput and the corresponding statistics table and Section \ref {sec:corr:churn} shows the correlation table for churn and the corresponding descriptive statistics table.


The content of the sections will consist of highlighting the variables with a significant correlation and variables that stands out. To back up any assumptions about the variables, descriptive statistic tables stated in appendix \ref{app:DS} and graphs will be used . The correlation relationship between two variables will occur twice, in both the correlation tables, since correlation is symmetric. When the relationship is highlighted in one of the sections, the relationship will not be highlighted in the other section. 




Since ten percent of leadtime and churn is removed, these two tables are combined and can consist of up to 184 dates per quarter - Skal jeg skrive det?

The descriptive statistics tables \ref{DS:corr:WIP}, \ref{DS:corr:LT}, \ref{DS:corr:Bugs},  \ref{DS:corr:TP} and \ref{DS:corr:Churn}  shows number(N), mean, median standard deviation(Std.dev), maximum(max)  and minimum(min) values from on the correlation table.  These values are based on all the teams values. For example, in table \ref{DS:corr:WIP}, one can see that all the teams in SI combined have a mean of value of 0.6 for throughput, when talking about correlation between WIP and throughput. 

\section{Correlation - WIP}
\label{sec:corr:WIP}
In this section the Table  \ref{corr:WIP}  shows the correlation between WIP and the variables for the teams. The table shows team one, three, four, six, seven, nine and ten have significant positive relationship between WIP and throughput. Since throughput feature and throughput bug is subset of throughput, its natural that these variables also have a significant positive correlation to WIP. But, for team one, six, seven and ten that's not the case. 
...
Bugs &.73**& 0.20& .60*& .56*& 0.50& 0.46& -0.07& 0.04& t.58*& 0.18\\ \hline
...
Both throughput and throughput feature in team one's case have significant positive relationship with WIP, throughput bug do not. Throughput and throughput feature have the correlation values .74 and .73 while throughput bug's value is 0.02. The reason why throughput bug do not have a close relationship to throughput maybe because throughput bug consist 37\% (108/290) of throughput as shown in the total row in tables \ref{DS:Throughput:1} and \ref{DS:TPB:1}. But it's possible to have a close relationship even though, since the correlation is based on the mean values. But throughput feature have a total mean of 13.7, while throughput bug have 6.4 as shown in tables \ref{DS:TPFT:1} and \ref{DS:TPB:1}. The total mean of throughput is 11 as shown in table \ref{DS:Throughput:1}. The mean values strengthens the assumption that throughput feature represents most of the throughput variable. The figures in Figure \ref{corr:Difference:1} proves the assumption. The pattern of the dots in the Figures \ref{fig:a:1} and \ref{fig:c:1} are similar. They both increase when the X values does.  While the dots in Figure \ref{fig:b:1} sometimes decrease when X do and sometimes decreases. 

\begin{figure}[h]
\centering     %%% not \center
\subfigure[WIP and Throughput]{\label{fig:a:1}\includegraphics[scale=0.5]{Picture/One/WIPvsTP.png}}
\subfigure[WIP and Throughput feature]{\label{fig:b:1}\includegraphics[scale=0.5]{Picture/One/WIPvsTPFT.png}}
\subfigure[WIP and Throughput bug]{\label{fig:c:1}\includegraphics[scale=0.5]{Picture/One/WIPvsTPB.png}}
...
The same issue encounter for both team six and seven. In case of team six, both throughput and throughput feature have significant positive relationship with WIP while throughput bug do not. Throughput, throughput feature and throughput bug have the correlation values .64, .68 and 0.07. The total row in tables \ref{DS:Throughput:6},  \ref{DS:TPFT:6} and \ref{DS:TPB:6} shows that throughput feature consist of 609 dates and a mean value of 4.8, while throughput bug consist of 82 dates  and a mean of 3.3. The throughout variable consist of 691 dates and have a mean of 4.58 which reflects that throughput feature contribute more to throughput than throughput bug, hence the strong relationship.


For team seven's case, the difference between throughput, throughput feature and throughput bug is so small, which also is indicated by the total row in tables \ref{DS:Throughput:7},  \ref{DS:TPFT:7} and \ref{DS:TPB:7}. In the total rows throughput have a total mean of 2.7, while throughput feature have 2.8 and throughput bug have 2.6. Throughput feature contributes 156 dates to throughput and throughput bug contributes 172 dates, so there is no need to explain the difference.

Team ten has significant positive relationship between WIP and the variables throughput and throughput bug. Throughput for team ten consist of 404 tasks as show in the total row in Table \ref{DS:Throughput:7}. Throughput bug represents 335 of these dates while throughput feature stands for 69 dates as shown in Tables \ref{DS:TPFT:7} and \ref{DS:TPB:7}. But the overall mean for throughput, throughput feature and throughput bug is 2.2, 2.3 and 2.2, which could reflect that these three variables should close. The Figures in \ref{corr:Difference:10} disproves that assumption. The figure shows that the correlation dots between WIP and throughput and WIP and throughput bug is almost identical, which proves that throughput bug have a close relationship to throughput. 


\begin{figure}[h]
\centering     %%% not \center
\subfigure[WIP and Throughput]{\label{fig:a}\includegraphics[scale=0.5]{Picture/Ten/WIPvsTP.png}}
\subfigure[WIP and Throughput feature]{\label{fig:b}\includegraphics[scale=0.5]{Picture/Ten/WIPvsTPFT.png}}
\subfigure[WIP and Throughput bug]{\label{fig:c}\includegraphics[scale=0.5]{Picture/Ten/WIPvsTPB.png}}
...
Team one, three, four and nine have a significant positive relationship between WIP and bugs as well. Team three and six have a significant positive relationship between WIP and churn bug. Why churn bug differ from churn for team six is stated in section \ref{sec:corr:lt}. But for team three, churn bug do not differ much from churn. Churn has a correlation of -0.47, churn bug have -.57 and churn feature have -0.35.  The tables \ref{DS:Churn:3}, \ref{DS:CF:3} and \ref{DS:CB:3} shows that churn have a total mean of 59.4, churn feature have 102.6 and churn bug 41.1. This shows that churn bug contributes more to churn than churn feature, since the mean of churn is closer to churn bug. 

Team four is the it-support team for SI.  IT-support do not have churn, since they don't code and that reflects why churn, churn feature and churn bug is indicated with a .c in Table \ref{corr:WIP}. But, team four have a significant positive relationship between WIP and the rest of the variables. 
...
\label{sec:corr:lt}
In this section the table \ref{corr:Leadtime} shows the correlation between leadtime and the variables. Team two, six and ten have significant negative relationship between leadtime and churn bug. The churn and churn bug variables for team two have the values of  -0.45 and -.73. These values are relative close.  However, churn feature have a value of 0.41. The total row in tables \ref{DS:Churn:2}, \ref{DS:CF:2} and \ref{DS:CB:2} shows that churn for team two consist of 1572 dates, churn feature of 375 dates and churn bug of 1197. Churn bug is 76\%  (1197/1572) of the total churn. The total mean of churn, churn feature and churn bug is 65.7, 111.4 and 51.3, which reflects that churn bug contribute more to the churn variable than churn feature, hence the two mean values are close.  Team two also has significant negative relationship between leadtime and bugs finished, quarter variable.

The churn bug for team six have a negative correlation of -.55 while churn have a correlation of -0.08 and churn feature a correlation of -0.2. The reason why churn bug differ from churn is because churn bug consist of 17\% (265/1533) of the churn variable as shown in tables \ref{DS:Churn:6} and \ref{DS:CB:6}. The same tables shows that the total mean of churn is 146, while the total mean for churn feature is 156.5 and for churn bug it's 95.2. The total mean concludes that churn bug don't contribute to much to the churn variable, hence big difference of the two mean values. Why throughput bug differ from both throughput where explained in section \ref{corr:WIP}. 
\begin{table}[!htbp]
 \begin{adjustwidth}{-2.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
\end{table}
\newpage
Churn bug is not the only churn variable that has a significant negative relationship between leadtime for team ten, churn do as well. Then the assumption is that churn feature does not have a close relationship with churn. The Tables \ref{DS:Churn:10}, \ref{DS:CF:10} and \ref{DS:CB:10} shows the total mean of churn is 50.4, for churn feature it's 66 and for churn bug it is 47.7. This shows that churn and churn bug total mean is close, but the churn feature is not so far away. Churn bug also consist of 85\% of the overall dates in churn (517/609) which supports the assumption that churn bug consist of much of the churn variable. The assumption is proven right by the Figure \ref{corr:Difference:lt}. One can clearly see that Figure \ref{fig:a:lt:10} and \ref{fig:c:lt:10} have a significant negative correlation. The dot pattern in these two figures is also similar,  which points to the case that churn and churn bug are alike and that churn feature don't consist of much of the churn or have a close relationship to churn. 

\begin{figure}[h]
\centering     %%% not \center
\subfigure[Leadtime and Churn]{\label{fig:a:lt:10}\includegraphics[scale=0.5]{Picture/Ten/LTvsChrn.png}}
\subfigure[Leadtime and Churn feature]{\label{fig:b:lt:10}\includegraphics[scale=0.5]{Picture/Ten/LTvsChrnFT.png}}
\subfigure[Leadtime and Churn bug]{\label{fig:c:lt:10}\includegraphics[scale=0.5]{Picture/Ten/LTvsChrnB.png}}
\caption{Correlation graphs between leadtime and churn variables for team ten.}
\label{corr:Difference:lt}
\end{figure}

The descriptive statistic table for leadtime shown in table \ref{DS:corr:LT} shows that there is no value with overall significant relationship with leadtime. 
\begin{table}[!htbp]
 \centering
 \begin{tabular}{ | l | r | r | r | r | r | r | }
 \hline
...
\label{sec:corr:bug}
This section contains information about the correlation table between the variables and bugs. The result is listed in table \ref{corr:bug}. Teams one, two, three, four, five, nine and ten all have a significant positive correlation between bugs and throughput. Team three, four, five and nine all have a significant positive correlation between bugs and the three throughput variables. Why the relationship between the throughput variables differ for team one and ten is stated in section \ref{sec:corr:WIP}. 
\begin{table}[!htbp]
 \begin{adjustwidth}{-2.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
Throughput feature differ from throughput for team two. One could believe that the reason is that throughput bug consists of 2/3 of the throughput variable (460/690) as shown in the descriptive statistic Tables \ref{DS:Throughput:2}, \ref{DS:TPFT:2} and \ref{DS:TPB:2}. But the tables shows that the total mean of throughput is 4.39. While for throughput bug it's 4.8 and 3.7 for throughput feature. These three variables are quiet close, which should reflect that these three variables should be close based on correlation measurement. But, the Figures in \ref{corr:Difference:2} shows that they are not. Both Figures \ref{fig:a:2} and \ref{fig:c:2} shows that when x increases, y also increases. But for Figure \ref{fig:b:2} one can see that the dots are more randomly placed. For example, when the Y value is at vertex, x value is low.  When the two highest values of x, y values are both low and high.  

\begin{figure}[h]
\centering     %%% not \center
\subfigure[Bug and Throughput]{\label{fig:a:2}\includegraphics[scale=0.5]{Picture/Two/BugVSTP.png}}
\subfigure[Bug and Throughput feature]{\label{fig:b:2}\includegraphics[scale=0.5]{Picture/Two/BugVSTPFT.png}}
\subfigure[Bug and Throughput bug]{\label{fig:c:2}\includegraphics[scale=0.5]{Picture/Two/BugVSTPB.png}}
\caption{Correlation graphs between Bug and the throughput variables.}
\label{corr:Difference:2}
\end{figure}
\newpage

Team six and nine have a significant positive correlation between the variable Bugs finished, quarter. Team nine also has a negative correlation between bug and churn feature. Churn feature consist of approximately 1/3 (295/923) of the churn variable as shown in tables \ref{DS:Churn:9} and \ref{DS:CF:9}.  The total mean of churn feature is 133.9, the total mean for churn is 81.9 and the total mean for churn bug is 57.5 as shown in the previous stated tables and table \ref{DS:CB:10}, which means that there are fewer and more code required tasks for churn feature. This reflects why there is not a close relationship between churn and churn feature as well as churn and churn bug. 


In table \ref{DS:corr:Bugs} one can see that WIP with a mean value of 0.4 have a medium correlation with bugs as stated in Section \ref{corr:WIP}.  Throughput  have a value of 0.6. This indicates that throughput have a high correlation with bugs.
...
\label{sec:corr:TP}
This section contains information about the correlation table between throughput and the variables. The result is listed in Table \ref{corr:TP}. Throughput has a significant correlation to either throughput feature or throughput bug for each of the teams. But for some teams case, both throughput bug and throughput feature don't have a significant correlation. Why throughput feature differ from throughput for team one and six is explained in section \ref{sec:corr:WIP}.   Why throughput bug differ from throughput for team ten is also stated in section \ref{sec:corr:WIP}.  Why throughput feature differ from throughput for team two is stated in section \ref{sec:corr:bug}. 

For team eight, throughput bug differ from throughput. Throughput bug consist of 99 dates and have total mean of 1.5. Throughput feature consist of 92 dates and a mean of 3.2. While throughput have a mean of 2.3. These numbers are obtained from the descriptive statistic Tables \ref{DS:Throughput:8}, \ref{DS:TPFT:8} and \ref{DS:TPB:8}.  On the basis of these numbers it will look like both of the sub variables of throughput contributes and both of them should have close relationship to throughput.  But, in Figure \ref{corr:Difference:8} the difference are stated. In Figure \ref{fig:a:8} the dots are clearly moving in a upwards direction, while in Figure \ref{fig:b:8} all the dots are all gathered around the low values of Y  without one dot who are located in the top right corner. 


\begin{figure}[h]
\centering     %%% not \center
\subfigure[Bug and Throughput]{\label{fig:a:8}\includegraphics[scale=0.5]{Picture/Eight/TPvsTPFT.png}}
\subfigure[Bug and Throughput feature]{\label{fig:b:8}\includegraphics[scale=0.5]{Picture/Eight/TPvsTPB.png}}
...
Both team nine and ten have a positive significant correlation between bug and Bugs finished, quarter. Team six have a significant correlation between bugs and churn bug. Why churn bug differ from churn and churn feature is stated in section \ref{sec:corr:lt}. Team seven has a significant positive correlation between throughput and the variables churn and churn bug, but not churn feature. Churn feature has almost a significant positive correlation with throughput. Even though churn feature and churn bug have a positive correlation with churn as stated in section \ref{sec:corr:churn}, it not so that correlation is transitive \parencite{corr:transitive}. Bilder her?!

 
Team eight has a significant negative correlation between throughput and churn. Churn has a negative value of -.64, while churn feature has a value of -0.45 and churn bug has a value of -0.28. Churn feature consist of 246 of 329 tasks, as shown in Tables \ref{DS:Churn:8} and \ref{DS:CF:8}. The total mean of these three values are 0.9 for churn, 0.1 for churn feature and 3.6 for churn bug as shown in the two tables above and Table \ref{DS:CB:8}. These variables reflects that churn feature's value is not too far away from churn value. Churn feature contains several quarters with zero churn, which is why churn feature doesn't have a strong correlation with throughput. 

According to Table \ref{DS:corr:TP}, bugs have a strong positive correlation with throughput while WIP have a medium positive relationship with throughput.
...
Why churn feature for team two and ten differ from churn is stated in Section \ref{sec:corr:lt}. The reason churn bug differ in the case of team six is also stated in Section \ref{sec:corr:lt}. Team five on the other hand,  has a significant positive correlation between churn and churn bug, but not for churn feature.  Churn feature consist of approximately 10\% of the total churn value as shown in Tables \ref{DS:Churn:5} and \ref{DS:CF:5}. The two tables and Table \ref{DS:CB:5} shows that churn has a mean value of 23.5, churn feature with a mean value of 43.1 and churn bug with a mean value of 21.4. These values show that churn bug and have a close relationship with churn.
...
As stated in Section \ref{sec:corr:TP}, churn have a significant correlation to throughput for team eight. And in Section \ref{sec:corr:TP} it is stated churn and churn feature have a closer relationship than churn and churn bug. But in the correlation table for churn, that's not the case. Churn bug have a signifigtant positive relationship of .82 with churn. Although churn feature consist of approximately 75\% (246/329) of all churn dates as shown in Tables \ref{DS:Churn:8} and \ref{DS:CF:8}. But, also as stated in in \ref{sec:corr:TP}, almost each of the churn feature columns are zero, and the correlation measurement is based on the mean value. Which is why churn bug have a significant correlation to churn.


%Team seven: Churn bug consist of almost 2/3 of churn (417/652) as shown in tables \ref{DS:Churn:7} and \ref{DS:CB:7}. But the total mean for the three values is 97.1 for churn, 148.1 for churn feature and 68.4 for churn bug. The total mean shows that churn feature contribute more to the churn variable than churn bug. This shows in the correlation table for churn in section \ref{sec:corr:churn}
\newpage
As expected, shown in table \ref{DS:corr:Churn} both churn feature and churn bug on company level have a close relationship with churn
\begin{table}[!htbp]
 \centering
 \begin{tabular}{ | l | r | r | r | r | r | r | }
 \hline
...
\subfigure[Descriptive Statistic -WIP]{
 ...
 \hline
 Quarter &	N &	Mean &	Median & Std.Dev & Max	& Min \\ \hline
2010-3  & 25.0 & 14.1 & 14.0 & 5.1 & 27.0 & 9.0\\ \hline
2010-4 & 92.0 & 23.0 & 23.5 & 7.8 & 48.0 & 12.0\\ \hline
2011-1 & 90.0 & 14.8 & 13.5 & 5.0 & 26.0 & 6.0\\ \hline
2011-2 & 91.0 & 12.8 & 12.0 & 4.6 & 22.0 & 3.0\\ \hline
2011-3 & 92.0 & 17.2 & 16.0 & 6.5 & 32.0 & 8.0\\ \hline
2011-4 & 92.0 & 17.5 & 17.0 & 6.3 & 30.0 & 4.0\\ \hline
2012-1 & 91.0 & 14.3 & 15.0 & 3.9 & 24.0 & 7.0\\ \hline
2012-2 & 91.0 & 16.5 & 14.0 & 6.0 & 34.0 & 10.0\\ \hline
2012-3 & 92.0 & 23.8 & 20.0 & 10.4 & 43.0 & 7.0\\ \hline
2012-4 & 67.0 & 1.7 & 0 & 3.0 & 11.0 & 0\\ \hline
Total & 823.0 & 16.1 & 15.0 & 8.4 & 48.0 & 0\\ \hline
\end{tabular}
}
}
\subfigure[Descriptive Statistic -Throughput]{
