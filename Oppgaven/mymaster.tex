...
In this chapter the results are presented. The results are presented with five correlation tables and five corresponding descriptive statistics tables. Vertically in the correlation tables, the correlation variables are stated. Horizontally is the corresponding teams are stated. The team names are shortened in order to better fit the tables. Team one is shortened to T1, team two is shortened to T2 and so on. The correlation tables is based on the mean value from the teams variables. Some of the variables are shown in Table \ref{des} and are explained in chapter \ref{ch:DCC}, the rest will be explained here. Variables with suffix of "Feature" means tasks labeled with type "feature" are the only one who is counted. The same goes for variables with suffix "Bug". The "Bugs finished, quarter" variable represents how many tasks labeled "Bug" that are finished within the same quarter as it was created.  The "Avg days backlog, bug" variable represent the average number of days bugs are in backlog before it pulled out. These two variables are used as an indicator of the workload together with churn.   


This chapter will be presented in the following manner. Each of the sections contains a correlation table and a corresponding descriptive statistic table. The descriptive statistic tables present the correlation table with descriptive statistic.  The first Section \ref{sec:corr:WIP}, shows the tables for WIP, Section \ref{sec:corr:lt}, shows the tables for lead time, Section \ref{sec:corr:bug}, shows the tables for bug, Section \ref {sec:corr:TP}, shows the tables for throughput and Section \ref {sec:corr:churn}, shows the tables for churn.


The content of the sections will consist of highlighting the variables with a significant correlation and variables that stands out. To back up any assumptions about the variables, descriptive statistic tables for each of the variables for each of the teams stated in Appendix \ref{app:DS} and graphs will be used . The correlation relationship between two variables will occur twice, in both the correlation tables, since correlation is symmetric. When the relationship is highlighted in one of the sections, the relationship will not be highlighted in the other section. 


%Since ten percent of lead time and churn is removed, these two tables are combined and can consist of up to 184 dates per quarter - Skal jeg skrive det?

The descriptive statistics tables \ref{DS:corr:WIP}, \ref{DS:corr:LT}, \ref{DS:corr:Bugs},  \ref{DS:corr:TP} and \ref{DS:corr:Churn}  shows number(N), mean, median standard deviation(Std.dev), maximum(max)  and minimum(min) values from on the correlation table.  The values in the descriptive statistics tables are summary from the correlation table. For example, in table \ref{DS:corr:WIP}, one can see that all the teams in SI combined have a mean of value of 0.6 for throughput, when talking about correlation between WIP and throughput.
 

\section{Correlation - WIP}
\label{sec:corr:WIP}
This section contains the Table  \ref{corr:WIP}. The table shows the correlation between WIP and the variables for the teams. The table shows team one, three, four, six, seven, nine and ten have significant positive relationship between WIP and throughput. Since throughput feature and throughput bug is subset of throughput, its natural that these variables also have a significant positive correlation to WIP. But, for team one, six, seven and ten that's not the case. 
...
Lead time  & 10.0 & 0.1 & 0.1 & 0.3 & 0.7 & -0.3\\ \hline
...
\begin{figure}[h]
\centering     %%% not \center
\subfigure[Lead time and Churn]{\label{fig:a:lt:10}\includegraphics[scale=0.5]{Picture/Ten/LTvsChrn.png}}
\subfigure[Lead time and Churn feature]{\label{fig:b:lt:10}\includegraphics[scale=0.5]{Picture/Ten/LTvsChrnFT.png}}
\subfigure[Lead time and Churn bug]{\label{fig:c:lt:10}\includegraphics[scale=0.5]{Picture/Ten/LTvsChrnB.png}}
...
\begin{figure}[h]
\centering     %%% not \center
\subfigure[Bugs and Throughput]{\label{fig:a:2}\includegraphics[scale=0.5]{Picture/Two/BugVSTP.png}}
\subfigure[Bugs and Throughput feature]{\label{fig:b:2}\includegraphics[scale=0.5]{Picture/Two/BugVSTPFT.png}}
\subfigure[Bugs and Throughput bug]{\label{fig:c:2}\includegraphics[scale=0.5]{Picture/Two/BugVSTPB.png}}
...
\begin{figure}[h]
\centering     %%% not \center
\subfigure[Bug and Churn]{\label{fig:a:9}\includegraphics[scale=0.5]{Picture/Nine/BugsVSChrn.png}}
\subfigure[Bug and Churn feature]{\label{fig:b:9}\includegraphics[scale=0.5]{Picture/Nine/BugsVSChrnFT.png}}
\subfigure[Bug and Churn bug]{\label{fig:c:9}\includegraphics[scale=0.5]{Picture/Nine/BugsVSChrnB.png}}
...
\begin{figure}[h]
\centering     %%% not \center
\subfigure[Throughput and Throughput feature]{\label{fig:a:8}\includegraphics[scale=0.5]{Picture/Eight/TPvsTPFT.png}}
\subfigure[Throughput and Throughput bug]{\label{fig:b:8}\includegraphics[scale=0.5]{Picture/Eight/TPvsTPB.png}}
