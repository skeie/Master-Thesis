...
\label{chap:intro}
\section{Motivation}
In software development the processes and methods are important in order to deliver the right product on time. In software development one rarely solve two identical problems for different stakeholders. And the problems are getting bigger and more complex, which means that the process and methods needs to adapt as well.  The number of popular software development methods (e.g. Extreme programming, Spiral, Scrum and Kanban) in the recent years proves the assumption.This is why this thesis will focus on software development method, because the method in each development project is such a key element. Within software development methods, the Kanban method and the tool Work In Progress (WIP) -limit will be the main focus of this thesis. In Kanban the WIP-limit is used to limit the number of tasks each developer can work on to prevent bottlenecks and to ensure flow of tasks through the development cycle. There are published various literature on Kanban in software development such as "Kanban: Successful Evolutionary Change for Your Technology Business" \parencite{0984521402}, "Kanban and Scrum - making the most of both"  \parencite{Kniberg} and "Lean Software Management: BBC Worldwide Case Study" \parencite{Joyce}. Even though there is various literature there is no information on how to apply WIP-limit, even through most of the experience Kanban people agree that WIP-limit is an important tool.  There is no research backing the statement. There exist no clear rule or research on how to apply WIP-limit.  The literature states that one should experiment with WIP-limits in order to find the best WIP-limit for one's case.Since there is lack of available research on WIP-limit I choose to focus this thesis on the subject.
...
\item[Chapter \ref{chap:Background}: \nameref{chap:Background}:] \hfill \\  The second item
\item[Chapter \ref{chap:RM}: \nameref{chap:RM}:] \hfill \\  The third etc \ldots
