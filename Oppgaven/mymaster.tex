...
Lead time could be an essential ingredient when you look for the optimal WIP, if there is one.  Often in a project, lead time is split into pieces, so every task has its own lead time. This gives the development teams the advantages to experiment with different WIPs in order to see the different lead times and then measure which WIP that suits this project the best. 
...
\label{fig:b:7}
  \end{subfigure}
\caption{Correlation graphs between the churn variables.}
...
\section{WIP vs lead time}
As stated in Section \ref{sec:in:lt}, lead time could be an important tool to measure productivity. Each development process would like to get their lead time as low as possible. WIP is a principle decrease lead time.  The overall result of this study shows that when WIP decreases, so does lead time. Team one, four, five, seven and ten has significant positive correlation between WIP and lead time and both team two and three has a medium correlation as shown in Table \ref{DS:corr:WIP}. 



One could argue if number of team members increase, the number of work in progress also increases. But that should not have an impact on lead timem since with more employees, there is possible to work on more tasks.  The Table \ref{DS:corr:WIP:Team:LT} shows that over a period of 14 quarters, 
...
Team one 23, four4, five 20, seven 7 and ten 12 
