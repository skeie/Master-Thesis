...
\chapter{Results}                     
\label{ch:res}
The first correlation result showed a medium-high correlation relationship between team size and both \textit{WIP} and \textit{throughput}. Based on the correlation values, it's was hard to find any evidence if WIP limit matter in software development. This resulted in a new analyze where each variable for each quarter was divided by their team size. 

Each section except team size are presented with two correlation tables and two corresponding descriptive statistics tables, one correlation table and descriptive statistic table form the first analyze and the same from the second analyze. Team size is presented with one correlation table and one descriptive statistic table. The content of the sections will consist of highlighting the variables with a significant correlation and variables that stands out. To back up any assumptions about the variables, descriptive statistic tables listed in Appendix \ref{app:DS} and correlation graphs will be used. 

\section{Correlation - WIP}
\label{sec:corr:WIP}
Table \ref{corr:WIP} shows the correlation table for WIP. The teams variables are shown vertically in the correlation table. Horizontally are the corresponding teams. The team names are shortened. Team one is shortened to T1, team two is shortened to T2 and so on.
...
Team one, four, five, seven and ten has a positive correlation \textit{WIP} and \textit{lead time}.
Team four also has a positive correlation between each of the variables except \textit{team size}.
...
For team three is \textit{throughput bug} close to a significant positive correlation with a value of 0.52, while both \textit{throughout} and \textit{throughput feature} has the values .57 and .71. With these correlation values, it looks like \textit{throughput bug} represents most of the \textit{churn} variable, even through \textit{churn bug's} value is not a significant. The descriptive statistic tables empower the assumption. \textit{Throughput} contains 542 dates and has a total mean value of 3.7. \textit{Throughput feature} represents 200 of these dates, and has a mean value of 3.3, while \textit{throughput bug} represents the remaining 342 dates, and has a total mean value of 4.0, shown in tables \ref{DS:Throughput:3}, \ref{DS:TPFT:3} and \ref{DS:TPB:3}. The \textit{throughput} correlation Table \ref{corr:TP:V2} proves the assumption with a correlation value of 0.90 for \textit{throughput feature} and a value of 0.98 for \textit{throughput bug}

Team four has a signification positive correlation for all the \textit{throughputs} variables, \textit{bugs finished, quarter}, \textit{lead time}, \textit{churn} and \textit{churn feature}.  The reason churn bug differ is stated in section \ref{sec:corr:lt}. For team two differ \textit{throughput feature} based on the correlation values in  \ref{corr:WIP:V2}. The reason for this is stated in Section \ref{sec:corr:bug}. For team eight differ \textit{churn feature} from \textit{churn} based on the correlation value in Table \ref{corr:WIP:V2}. The reason is stated in Section \ref{sec:corr:lt}. Team nine has a significant positive correlation for all \textit{throughput} variables. Team nine has a significant positive correlation for all the \textit{throughput} variables. In Table \ref{DS:corr:WIP:v2}, \textit{throughput} has a overall correlation value of 0.4, while rest of the variables except \textit{throughputs} sub variables has a correlation values 0.2 or less. 
...
Team three and seven has a significant positive correlation between \textit{lead time} and \textit{bugs}. Team seven also has a significant positive correlation between \textit{lead time} and \textit{bugs finished, quarter}. The descriptive statistic table for lead time shown in Table \ref{DS:corr:LT} shows that both \textit{WIP} and \textit{throughput} has a medium to strong correlation with \textit{lead time}. 
\begin{table}[!htbp]
 \centering
 \begin{tabular}{ | l | r | r | r | r | r | r | }
 \hline
...
Team one has a significant positive correlation for \textit{bugs}, \textit{bugs finished, quarter}, \textit{churn} and \textit{churn bug}. The reason \textit{churn feature} differ form churn in respect of the correlation values in Table \ref{corr:LT:V2} is explained in Section \ref{sec:corr:WIP}. Team two has a significant positive correlation for  \textit{WIP}, the \textit{throughput} variables, \textit{bugs}, \textit{bugs finished, quarter} and \textit{churn bug}. The relationship between the \textit{churn} variables is explained i section \ref{sec:corr:WIP}. Team four has a significant positive correlation between \textit{WIP}, the \textit{throughput} variables, \textit{churn} and \textit{churn feature}. The \textit{churn} relationship for team four is explained in Section \ref{sec:corr:lt}.
\begin{table}
  \begin{adjustwidth}{-2.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
Team two's \textit{throughput feature} differ from \textit{throughput} based on the correlation values form Table \ref{corr:bug}. One could believe that the reason is that \textit{throughput bug} consists of 2/3 of \textit{throughput} (460/690) as shown in the descriptive statistic Tables \ref{DS:Throughput:2} and \ref{DS:TPB:2}. But the two tables and Table \ref{DS:TPFT:2} shows that the total mean of \textit{throughput} is 4.39. While for \textit{throughput bug} it's 4.8 and 3.7 for \textit{throughput feature}. These three variables are quiet close, which could reflect that these three variables could be close, based on correlation measurement. 
But, the Figures in \ref{corr:Difference:2} and the throughput correlation table in \ref{sec:corr:TP} shows that they are not. The Figure \ref{corr:Difference:2} shows the bugs variable in the X-axis and the throughput variable in the Y-axis. Both the Figures \ref{fig:a:2} and ref{fig:c:2} shows a significant positive correlation, while Figure \ref{fig:b:2} shows dots that are more randomly placed, hence the correlation of 0. The throughput correlation Table \ref{corr:TP} shows a correlation relationship with a value of .97 for \textit{throughput bug} and  0.1 for \textit{throughput feature}.
\begin{figure}[h] 
  \begin{subfigure}[b]{0.3\textwidth}
  \center
...
\label{fig:b:2}
  ...
  \begin{subfigure}[b]{0.3\textwidth}
  \center
\includegraphics[scale=0.5]{Picture/Two/BugVSTPB}
...
Team nine has a significant positive correlation for \textit{throughput} and \textit{throughput bug}. The reason \textit{throughput feature} don't has a significant correlation while \textit{throughput} has it, in respect of the Table \ref{corr:Bugs:v2}, could be because \textit{throughput bug} represents most of the \textit{throughput} variable, as shown in Tables \ref{DS:Throughput:9}, \ref{DS:TPFT:9} and \ref{DS:TPB:9}. The \textit{throughput} variables contains 521 dates and has a mean value of 2.6. \textit{Throughput feature} represents 214 of these dates and has a total mean of 2.4, while \textit{throughput bug} represents the remaining 307 dates and has a total mean of 2.8. These variables also shows that both the \textit{throughput} sub variables contributes. The throughput correlation Table \ref{corr:TP} proves that both the attributes contributes, but \textit{throughput bug} contributes the most with a correlation value of 0.96, while \textit{throughput feature} has a correlation value of 0.88. 
...
\label{sec:corr:TP}
This section shows the correlation table between throughput and the variables. The result is listed in Table \ref{corr:TP}. \textit{Throughput} has a significant correlation to either \textit{throughput feature} or \textit{throughput bug} for each of the teams. For team three, four, five, seven and nine, both the sub variables of \textit{throughput} have significant positive correlation to \textit{throughput}. For team one and six, \textit{throughput feature} differ from \textit{throughput}, the reason were explained in Section \ref{sec:corr:WIP}.   The Section \ref{sec:corr:WIP} also explain the reason \textit{throughput bug} differ from \textit{throughput} for team ten.    \textit{Throughput feature} differ from \textit{throughput} for team two and the reason for that were stated in section \ref{sec:corr:bug}.
...
\end{tabular}
 ...
 The Table \ref{corr:TP:V2} shows that team one has a significant positive correlation for \textit{throughput feature}. The reason \textit{throughput} relation were stated in Section \ref{sec:corr:WIP}.Team two has a significant positive correlation for \textit{WIP}, both \textit{throughputs} sub variable, \textit{bugs}, \textit{bigs finished, quarter} and \textit{lead time}. Team three has a positive correlation for \textit{WIP}, both \textit{throughputs} sub variables and \textit{bugs}. Team four has a significant relationship for \textit{WIP}, both \textit{throughputs} variable, \textit{bugs finished, quarter}, \textit{lead time}, \textit{churn} and \textit{churn feature}. The \textit{churn} relationship were described in Section \ref{sec:corr:lt}. For team five has \textit{throughput bug}, \textit{bugs}, \textit{bugs finished, quarter}, \textit{avg, days in backlog, bugs}, \textit{lead time}, \textit{churn} and \textit{churn feature}. The \textit{churn} relationship is explained in Section \ref{sec:corr:churn}. The \textit{throughput} relationship were described in \ref{sec:corr:WIP}. 
 \begin{table}[!htbp]
  \begin{adjustwidth}{-1.5cm}{}
 \centering
 \begin{tabular}{|l|r|r|r|r|r|r|r|r|r|r|}
...
\chapter{Discussion}
\label{ch:dis}
In this chapter a discussion around the research questions, the different results and what literature says are presented. Write more about std.dev
...
\section{Team size}
As the two different results sections in Chapter \ref{ch:res} and the sections above shows, it's important to take team size into account when looking for relationship between variables such as \textit{WIP}, \textit{throughput}, \textit{lead time} and \textit{bugs}. If team size were left out as a variable in this case study, \textit{WIP} would had a linear relationship of 0.6 with \textit{throughput}, 0,4 with \textit{bugs} and 0.4 with \textit{lead time}. The three correlation values increased by 0.2 compared to when  team size was taken into account. These variables would have showed a stronger results towards the fact that WIP-limit matter in software development. 
...
The bugs correlation tables \ref{DS:corr:Bugs} and \ref{DS:corr:Bugs:v2} shows a difference in the correlation value between \textit{bugs} and \textit{WIP}. The correlation value is increased by 0.2 when team size is \textbf{not} taken into account. Both the \textit{throughput} correlation values are 0.6. The correlation value for \textit{lead time} is also increased by 0.2 when team size is \textbf{not} taken into account, while churn increased by 0.3. These values shows a impact of team size on the correlation relationship for 3 of 4 variables. 


For \textit{lead time} increases the churn value by 0.2 when time size is \textbf{not} taken into account, otherwise are the correlation values for \textit{lead time} stated preceding paragraphs. The values shows the importance of taking team size into account when a case study like this is conducted. 
...
\chapter{Conclusion}
\label{ch:conc}
In this thesis the main goal was to investigate the research question "Does WIP limit in software development matter?".  If so, further goals was to see "How to find the optimal WIP limit" and "Which parameters should be considered in order to optimize WIP".  To answer the research questions a data set from a company called Software Innovation were interpreted. The data set is based on metadata about tasks from 2010 to 2013.

If WIP limit matters in software development some of it's benefits should be to reduce bugs, increase throughput, decrease lead time ect. Based on the results from the case study, \textit{WIP} has an impact on \textit{throughput}. The results shows a positive linear relationship between \textit{WIP} and \textit{throughput}. The previous research has stated to lower WIP to decrease throughput. This research shows otherwise. Since \textit{throughput} decreased when \textit{WIP} does, and their is no evidence after this case study that lowering WIP increases \textit{throughput} or decreases \textit{lead time} or \textit{bugs}.  Since WIP - limits don't matter in software development based on this study, there is no need to find an optimal WIP-limit or know which parameters to take into account in order to optimize WIP. 

However, the results from this paper shows some other findings. The importance of taking team size into account when measuring a similar research, the churn variable has no or little impact on the software process.  

The values of this data, example lead time has a close relationship between order empower this validity of this study

\section{Future work}
The conclusion from this thesis is made on one case study. I would recommend to do the same calculation as in this thesis with another data set and compare the outcome. I would also suggest an different approach as looking more deeply into the relationship between \textit{WIP} and  \textit{team size}. I suggest one measure the number of employees working on each task and instead of take the number of employees per quarter and divide on the mean of each variables values.

I will also suggest to look at the values up against release dates. The number of WIP's, throughput, bugs et cetera usually decrease around release date,  which could cause bias in the data. There is also done another research on the data from SI, i would suggest to take the result from this case study and compare them with the results from the other research. 
