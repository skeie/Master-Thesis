...
Section \ref{WIPsec} states that WIP-limits should be in the range from 1 to 3 tasks per developer. The mean WIP-limit for SI is 1.9 per developer, as stated by Section \ref{sec:WIP:limit}. This shows that the WIP-limit  could be either too low or too high. The rest of the discussion in this section and Sections \ref{sec:dis:lt}, \ref{sec:dis:churn} and \ref{sec:dis:bugs} will explore the option that the WIP-limits may have been set too low or too high, and discuss if WIP-limits matter at all.  The correlation between WIP and the variables will be discussed as well as the correlation of $\pm$ 0.2 between variables. Based on the data from this data set, there is hard to find any pattern between the variables with mean correlation from 0.2 to -0.2, because these relationship have few significant correlations were the WIP-limit can be investigated in.
...
The mean correlation between \textit{lead time} and \textit{throughput} are 0.5, as showed by Table \ref{DS:corr:WIP:v2}. The positive correlation between these variables states that when \textit{throughput} increases, the time from when a task is taken from the backlog until it is finished also increases. The relationship also states that if \textit{throughput} decreases, then the \textit{lead time} also decreases.
...
The four teams with the lowest WIP-limits have the correlation of 0.06, 0.26, 0.24 and 0.90, showed by \ref{corr:WIP:V2} and Figure \ref{table:WIP:teams}. Three of the five teams with the significant correlations are also the three teams with the highest mean WIP-limits. Three of the four with the lowest WIP-limits also have the lowest correlation between \textit{lead time} and \textit{throughput}. This data shows that most of the teams with highest mean WIP-limits have a significant correlation and most of the lowest mean WIP-limits are the one closes to have a negative correlation between \textit{throughput} and \textit{lead time}. Although the data show evidence that WIP-limits reduce the correlation between \textit{lead time} and \textit{throughput}, there is no evidence that lowering WIP-limits even more will result in a negative correlation between \textit{lead time} and \textit{throughput}.  But, based on these data, it looks like WIP-limits matter, because it can decrease the correlation between \textit{lead time} and \textit{throughput}. Based on this as well, it looks like the WIP-limits may have been set too high. 
...
The former research states that WIP-limits have an impact on \textit{lead time}, the data from SI shows the same. The correlation between \textit{lead time} and \textit{WIP} showed evidence that WIP-limit matter. These data also showed evidence that the WIP-limit may have been set too low. The other correlations with a mean of $\pm$ 0.2 were investigated and no one of them showed a pattern towards the fact that WIP-limit helps \textit{lead time}.
...
The correlation between \textit{bugs} and \textit{bugs finished, quarter} should be positive. \textbf{Team one, six, seven, eight} and \textbf{ten} have a significant positive correlation between \textit{bugs} and \textit{bugs finished, quarter}. These teams have a mean WIP-limit of 0.73, 3.02, 2.03, 0.71 and 1.08, as shown by Table \ref{corr:Bugs:v2} and Figure \ref{table:WIP:teams}. These values show no pattern between WIP-limit  and the correlation between \textit{bugs} and \textit{bugs finished, quarter}. There are two teams with a correlation lower than 0.20 and these teams have the WIP-limit of 3.21 and 2.19. The three remaining teams have the correlation of 0.44, 0.50 and 0.52, showed by Table \ref{corr:Bugs:v2} and have the WIP-limits of 2.31, 1.78 and 1.93, as showed by Figures \ref{Team:WIP:2}, \ref{Team:WIP:3} and \ref{Team:WIP:5}.  The WIP-limit for the teams with the correlation of 0.44, 0.50 and 0.52 shows some sort of a pattern, these WIP-limits are close.
...
But, the conclusion is that it looks like WIP-limits do not matter in the correlation relationship between  \textit{bugs} and \textit{bugs finished, quarter}, because the teams with significant correlations have a high scattering between the mean WIP-limits.
...
Both the low and high WIP-limits have a pattern in their correlations, but the significant correlations have a scattered WIP-limits, this points towards the fact that the WIP-limits does not matter for the correlation between \textit{bugs finished, quarter} and \textit{churn}.
