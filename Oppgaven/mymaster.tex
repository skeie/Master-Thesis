...
If team size were not taken into account when the correlation tables showed in Chapter \ref{ch:res} were measured, \textit{WIP} would have a mean correlation relationship of 0.6 between \textit{throughput}. A mean correlation of 0.5 between \textit{WIP} and both \textit{bugs} and \textit{lead time}. These three correlations are decreased by 0.2  and 0.3 compared to when team size was taken into account.  \textit{WIP} has a mean correlation of 0.6 between \textit{team size}, \textit{throughput} has a mean correlation of 0.5 between \textit{team size}, \textit{bugs} has the mean correlation of 0.4 between \textit{team size} and \textit{lead time} has the mean correlation of 0.3 between \textit{team size}.  In light of the result from Chapter \ref{ch:res}, the results from when team size was \textbf{not} taken into account are discarded. Because the data showed that team size had a great impact on the correlation tables and possible gave biased results.
...
\vspace{-0.5em}
In this work, throughput is a measure to see how productive teams are. Section \ref{WIPsec} states by various people, when lowering WIP-limit the {throughput} increases. The paper  by David Anderson \parencite{DavidAnderson} simulated a  lean-kanban process. The simulation showed a higher throughput when WIP-limit was used. The paper by Concas \parencite{SMR:SMR1599} showed that WIP-limit could lead to improvement in throughput as well. The paper also showed that when taking a process without WIP-limit, and simulate it using WIP-limit it outperformed the process without WIP-limit These papers were stated in Section \ref{WIPsec}.

The literature says that there should exist a negative correlation between \textit{WIP} and \textit{throughput}. The correlation from this work shows that almost each team has a positive correlation between \textit{WIP} and \textit{throughput} and a mean correlation of 0.4, as shown by Tables \ref{corr:WIP:V2} and \ref{DS:corr:WIP:v2}. The reason \textit{WIP} and \textit{throughput} have a positive correlation could be because the WIP-limits is set to low or too high. Section \ref{WIPsec} states that WIP-limits should be in the range from one to three tasks per developer. The mean WIP-limit for SI is 1.9 per developer, as stated by Section \ref{sec:WIP:limit}. The mean WIP-limit  could be either to low or too high and that could be the reason \textit{throughput} and \textit{WIP} have a positive correlation relationship. The rest of this section will discuss the option that the WIP-limits may be set to low or too high and to if WIP-limits matter.  The correlation values of $\pm$ 0.2 between \textit{throughput} will be discussed. The mean correlations from 0.2 to -0.2 are discarded, because they do not have an impact.
%If the WIP-limits are too low, bottlenecks will not occur and the teams are able to produce more, but they are not allowed by the WIP-limit. If WIP-limits are too high, people could be multitasking which decreases \textit{throughput}. 
...
\textbf{Team four} has a mean throughput of 1.4 per employee and \textbf{team six} has a mean throughput of 0.63 per employee, as shown by Tables \ref{Team:TP:4} and \ref{Team:TP:6}. The  team with the third highest  throughput mean per employee is \textbf{team two} with a mean of 0.44 and a WIP-limit of 2.31, showed by Figures \ref{Team:TP:2} and \ref{Team:WIP:2}. The teams \textbf{one} and \textbf{five} have a mean throughput of 0.42 and 0.30 as showed by Figures \ref{Team:TP:1} and \ref{Team:TP:5}. This shows that team \textbf{four} outperform the other teams on throughput per developer and \textbf{team six} is second by a wide margin. These data points towards the fact that WIP-limit may be too low, because the three teams with the highest throughput per employee also have the highest WIP-limits. 

The \textit{WIP} correlation Table \ref{corr:WIP:V2} shows \textbf{team two} with a correlation of 0.59 between \textit{throughput} and \textit{WIP}, \textbf{team four} with a significant correlation of 0.86 between \textit{throughput} and \textit{WIP} and \textbf{team six} with a correlation of 0.08 between \textit{WIP} and \textit{throughput}.  These correlations show that WIP-limit may not be too low, because \textbf{team two} and \textbf{team four} have positive significant correlation and because \textbf{team four} outperformed the other teams on throughput per employee. The teams with the lowest correlation between \textit{throughput} and \textit{WIP} are; \textbf{team ten} with a correlation of -0.21 and a WIP-limit of 1.08, \textbf{team five} with a correlation of 0.11 and a WIP-limit of 1.93 and \textbf{team six} with a correlation of 0.08 and a WIP-limit of 3.02 as showed in Figures \ref{corr:WIP:V2}, \ref{Team:WIP:10}, \ref{Team:WIP:5} and \ref{Team:WIP:6}. Based on these values it will look like WIP-limits do not have an impact on the correlation between \textit{throughput} and  \textit{WIP}, because teams with both high and low WIP-limits have both high and low correlation between \textit{WIP} and \textit{throughput}.
...
The mean correlation between \textit{throughput} and \textit{churn} are 0.3, as showed by Table \ref{DS:corr:WIP:v2}. This states that if the tasks requires more effort, they are able to produce more. This correlation is odd, the correlation between these two variables should be a negative one.  There are three teams with a significant correlation, as showed by Table \ref{corr:WIP:V2}. These three teams have the mean WIP-limits of 3.21, 1.93 and 0.71, showed by Figure \ref{table:WIP:teams}. There are three teams with a negative correlation between \textit{throughput} and \textit{churn}. These negative correlations are -0.14, -0.14 and -0.28, as showed by Table \ref{corr:WIP:V2}. The teams with these negative correlations have the mean WIP-limits of 0.73, 3.02 and 2.19, as showed by Figure \ref{table:WIP:teams}. Both teams with a high mean WIP-limit and low WIP-limit have both a significant positive correlation and negative correlation.  This states that there is no pattern between WIP-limits and the correlation relationship between \textit{throughput} and \textit{churn}. 

The data in this section shows that WIP-limit does not matter in the correlation for \textit{throughput}.  The correlation between \textit{throughput} and \textit{lead time} showed evidence that the WIP-limits could be too high, but the other correlations showed no evidence of WIP-limits impact. 
...
\vspace{-1em}
As stated in Section \ref{sec:in:lt}, lead time could be used to track how quickly software is delivered to customers. Each development process would like to get their lead time as low as possible. Section \ref{WIPsec} states that WIP-limits are important to reduce lead times. The correlation between \textit{lead time} and \textit{WIP} shows a mean correlation of 0.2, shown by Table \ref{DS:corr:WIP:v2}. The positive correlation between \textit{lead time} and \textit{WIP} states that \textit{lead time} increases when \textit{WIP} increases. Based on former research, the correlation between \textit{WIP} and \textit{lead time} should be a negative one. 

The teams \textbf{two, four, six} and \textbf{seven} have a correlation of 0.67, 0.87, 0.32 and 0.77 and mean WIP-limits of 2.31, 3.21, 3.02 and 2. Teams \textbf{one, three, five, eight, nine} and \textbf{ten} have a correlation values from 0.03 to -0.18. These teams have a mean WIP-limit of 0.73, 1.78, 1.93, 0.71, 1.09 and 2.19. The correlation between \textit{WIP} and \textit{lead time} should be positive, so when WIP is decreased, then \textit{lead time} decrease as well. These data points towards the fact that WIP-limit may be too low, because four of the five teams with a WIP-limits of 2 or higher have a positive correlation of 0.32 or higher. 

The mean correlation between \textit{lead time} and \textit{bugs} are 0.5.  The correlation means that if SI produces less \textit{bugs}, the \textit{lead time} for each task will decrease, which is a good thing.  Five teams have a positive significant correlation and these teams have a WIP-limit of 0.73, 2.31, 1.91, 2.03 and 1.08.  This shows no pattern, there are both teams with high and low mean WIP-limits. The two with the lowest correlation between \textit{WIP} and \textit{lead time} are teams \textbf{six} and \textbf{eight} . They have the correlation of 0.-23, -0.13 and mean WIP-limits of 3.02 and 0.71. The WIP-limits 3.02 and 0.71 are almost the highest and the lowest WIP-limit in SI. Based on these data, WIP-limits do not make an impact one correlation between \textit{bugs} and \textit{lead time}, because there  is no pattern between the teams with high WIP-limit or low WIP-limit and the correlation between \textit{lead time} and \textit{bugs}.
...
The mean correlation between \textit{lead time} and \textit{churn} are 0.4. The correlation between \textit{lead time} and \textit{churn} should be positive. Teams \textbf{one, four, five} and \textbf{eight} have a significant positive correlations and these teams have mean WIP-limits of 0.73, 3.21, 1.93 and 0.71. The three teams with the lowest correlations are teams \textbf{six, nine} and \textbf{ten}, these three teams have the WIP-limits of 3.02, 2.19 and 1.08. The teams should have a positive correlation between  \textit{lead time} and \textit{churn}, such that the teams can work on small tasks and delivery quickly, which is one of the principle of Kanban (section ref). Based on the WIP-limits and the correlation, WIP-limits cannot be used to regulate the relationship between  \textit{lead time} and \textit{churn}, because there is no pattern between WIP-limits and the correlation values.

The mean correlation between \textit{lead time} and  \textit{bugs finished quarter} and are 0.4. This relationship means that if \textit{lead time} decreases, the number if bugs closed in each quarter decreases. The relationship between these variables should be negative, so one can close more bugs and minimize \textit{lead time}. Teams \textbf{one, two, five, seven} and \textbf{ten} have a significant positive correlation and these teams have the mean WIP-limits of 0.73, 2.31, 1.93, 2.03 and 1.08. There is one team with a negative correlation. That correlation is -0.04 and the team with that correlation has mean WIP-limit of 0.71. However, the two other teams with the lowest correlations have the correlations of 0.32 and 0.36. These two teams also have the WIP-limits of 1.78 and 3.02. These data points towards the fact that WIP-limits have no impact on the correlation between \textit{lead time} and \textit{bugs finished quarter}, because there is no pattern between high or low WIP-limits and the correlations.
...
The paper cited by Shinkle stated; when WIP-limits was too high, the number of bugs increased  \parencite{Shinkle}. The mean correlation between \textit{WIP} and \textit{bugs} are 0.2, which states that if WIP decreases, so does bugs. A positive correlation close to one between \textit{WIP} and \textit{bugs} should be the goal.  There is one team with a significant correlation and that is \textbf{team seven}  with a correlation of 0.75 as showed in Table \ref{corr:WIP:V2}. \textbf{Team seven} has  a mean WIP-limit of  2.03 shown by Table. The second highest correlation is 0.49 and belongs to \textbf{team two}. \textbf{Team two} has the correlation of 0.49 and a WIP-limit of 3.21.  The teams with the lowest correlation between \textit{WIP} and \textit{bugs} are \textbf{team nine} and \textbf{team ten}. \textbf{Team nine} has the correlation of -0.05  and a mean WIP-limit of 2.19 and \textbf{team ten} has the correlation of -0.28 and a mean WIP-limit of 1.08. The team with the highest WIP-limit, \textbf{team four} has the correlation value of 0.27 between \textit{WIP} and \textit{bugs}. The team with the lowest  WIP-limit, \textbf{team eight} has a correlation value of 0.32. These values shows that there is no pattern between \textit{WIP}-limits and \textit{bugs}.

The mean correlation between \textit{bugs} and \textit{bugs finished, quarter} are 0.5. This correlation states if SI produce more bugs, they are also able to close more bugs within each quarter. The \textit{bugs finished, quarter} value is measured in percents of closed tasks within a quarter. So if more bugs are produced, will not automatically say that more bugs are produced within the same quarter, just because they can produce more bugs. This correlation should be positive. Teams \textbf{one, six, seven, eight} and \textbf{ten} have a significant positive correlation between \textit{bugs} and \textit{bugs finished, quarter}. These teams have a mean WIP-limit of 0.73, 3.02, 2.03, 0.71 and 1.08. These values show no pattern between \textit{WIP-limit} and the correlation between \textit{bugs} and \textit{bugs finished, quarter}. 

The \textit{avg days in backlog, bugs} has a mean correlation of 0.4 between \textit{bugs}. This means that the more bugs produced, the longer the bugs stays in the backlog. This correlation should be negative. There is one team with a negative correlation. That team is \textbf{team two} with a correlation of -0.07 between \textit{avg days in backlog, bugs} and \textit{bugs}. \textbf{Team two} has a WIP-limit of 2.31.There are two teams with a significant correlation, these teams are \textbf{team one} and \textbf{team five}. These two teams have the correlation of 0.76 and 0.75 and WIP-limits of 0.73 and 1.93. One can not conclude anything out of these three values. So there are six teams with close correlations. These correlations are 0.30, 0.37, 0.38 and 0.39. These teams have the WIP-limits of 1.78, 3.02, 2.03, 0.71, 2.19 and 1.08. Based on these correlations and WIP-limits there are not pattern between high or low WIP-limit and high and low correlations.
...
%Based on the descriptive statistics tables, independently of  team size; there is no relationship between \textit{WIP} and \textit{churn} or the moderating variables of churn according to Tables \ref{DS:corr:WIP} and \ref{DS:corr:WIP:v2}. The results show a mean correlation of -0.1, 0 and 0.1 for the \textit{churn} variables. When team size is \textbf{not} taken into account, two of the teams have a significant correlation, one of them has a positive correlation and the other has a negative. When team size is taken into account, there is one team with a significant positive correlation.

%The standard deviation for both the results are 0.4, while the median is 0.1 when team size is \textbf{not} taken into account and -0.2 for the other case. The standard deviation empowers the assumption of a relationship between \textit{churn} and \textit{WIP}, because the values are spread and there is no pattern.  The Table \ref{corr:All} shows an overall correlation of 0.06 when team size is taken into account. The Table \ref{corr:All:V2} shows a significant correlation of 0.17. These values show that WIP-limit has little impact on \textit{churn}.
