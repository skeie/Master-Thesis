...
There exist two teams with a mean WIP-limit of 3 tasks per developer. These teams are  \textbf{team four} and \textbf{team six}, shown by Tables \ref{Team:WIP:4} and \ref{Team:WIP:6}. These two teams had a total number of 53 and 99 employees from 2010-4 to 2013-4, which gives a mean of 4 (53/14) and 7 (99/14) persons per quarter. There are no other teams with fewer employees per quarter than these two teams, although these two teams have a mean throughput of 6.2 for \textbf{team four} and 4.6 for \textbf{team six} as showed by the total mean row in the descriptive statistic Tables . There are only two teams with a higher mean throughout than these two teams and that is teams \textbf{one} and \textbf{five}, as shown by the total mean row in Tables. In comparison \textbf{team one} had in average 24 (330/14) employees per quarter and \textbf{team five} had in average 20 (285/14) employees per quarter. 

\textbf{Team four} has a mean throughput per employee of 1.4 and \textbf{team six} has a mean throughput of 0.63 per employee, as shown by Tables \ref{Team:TP:4} and \ref{Team:TP:6}. The  team with the third highest  throughput mean per employee is \textbf{team two} with a mean of 0.44 and a WIP-limit of 2.31. Teams \textbf{one} and \textbf{five} have a mean throughput of 0.42 and 0.30. This shows that team \textbf{four} outperform the other teams on throughput per developer and \textbf{team six} is second by a wide margin. These data points towards the fact that WIP-limit may be too low. 

The \textit{WIP} correlation Table \ref{corr:WIP:V2} shows \textbf{team four} with a significant correlation of 0.86 between \textit{throughput} and \textit{WIP} and \textbf{team six} with a correlation of 0.08 between \textit{WIP} and \textit{throughput}.  These correlations show that WIP-limit may not be too low, because \textbf{team four} has the positive significant correlation. The teams with the lowest correlation between \textit{throughput} and \textit{WIP} are; \textbf{team ten} with a correlation of -0.21 and a WIP-limit of 1.08, \textbf{team five} with a correlation of 0.11 and a WIP-limit of 1.93 and \textbf{team six} with a correlation of 0.08 and a WIP-limit of 3.02. Based on these values it will look like WIP-limits do not has an impact on the correlation between \textit{throughput} and  \textit{WIP}, because teams with both high and low WIP-limits have both high and low correlation between \textit{WIP} and \textit{throughput}. To further investigate this hypothesis, each variables with a correlation of $\pm$ 0.2 between \textit{throughput} will be discussed to see if there exist a pattern between these variables and \textit{WIP}-limits.
...
The mean correlation between \textit{lead time} and \textit{throughput} are 0.5.  The positive correlation between these variables state that when \textit{throughput} increases, the time from when a task is taken from the backlog until it is finished also increases.  This correlation relationship should be negative. Because if \textit{throughput} increases, the \textit{lead time} should decrease. There are five teams with a positive significant correlation. The mean WIP-limits for these five teams are 2.31, 3.21, 1.93, 3.02 and 1.08. However, the correlation 0.06 and 0.24 are the two lowest correlation between \textit{lead time} and \textit{WIP}, the two teams with these correlations have a mean WIP-limit of 0.73 and 0.71, which are the two lowest mean WIP-limit. Three of the five teams with a significant correlation are also the three teams with the highest mean WIP-limits. This data shows that the highest mean WIP-limits have a significant correlation and the lowest mean WIP-limits are the one closes to have a negative correlation between \textit{throughput} and \textit{lead time}. Based on these data, it looks like the WIP-limits may be too high. 

The mean correlation between \textit{bugs} and \textit{throughput} are 0.6. The positive correlation between \textit{bugs} and \textit{throughput} states that if SI produces more, then they produce more bugs. This correlation relationship is normal. When one produces more tasks, there are likely to produce more bugs, if one produces zero tasks, one also produces zero bugs. In an optimal process should the correlation between \textit{bugs} and \textit{throughput} have a negative correlation, so teams can produce more without producing bugs. This is the principle of \textit{JIT}, explained in Section \ref{sec:JIT}.


There are six teams with a significant positive correlation between \textit{throughput} and \textit{bugs}, these six teams have a mean WIP-limit of 2.31, 1.78, 1.93, 0.71, 2.19 and 1.08. The teams \textbf{one} and \textbf{six} are the only teams with a correlation lower than 0.45. \textbf{Team one} has a correlation of 0.05 and a WIP-limit of 0.73. \textbf{Team six} has a correlation of -0.02 and a mean WIP-limit of 3.02.  Based on these data, there is no pattern that \textit{WIP}-limits will create a negative correlation between \textit{throughput} and \textit{bugs}, because teams with both high and low mean WIP-limit have a significant correlation between \textit{throughput} and \textit{bugs}.  

The mean correlation between \textit{throughput} and \textit{bugs finished, quarter} are 0.4. The mean correlation between \textit{throughput} and \textit{throughput bug} are 0.8 and the mean correlation between \textit{throughput} and \textit{throughput feature} are 0.7. The positive correlation between  \textit{throughput} and \textit{bugs finished, quarter} states that when SI produces more, they are able to fix more bugs within the same quarter, which shows the same as the correlation between \textit{bugs} and \textit{throughput}. The positive correlation between \textit{throughput} and both the moderating variables  states that SI produces more bugs than feature. These correlations show the same as the correlation between \textit{throughput} and \textit{bugs}. 


The mean correlation between \textit{throughput} and \textit{churn} are 0.3, which states that if the tasks requires more effort, they are able to produce more. This correlation is odd, the correlation between these two variables should be a negative one.  There are three teams with a significant correlation, these three teams have mean WIP-limits of 3.21, 1.93 and 0.71. There are three teams with a negative correlation between \textit{throughput} and \textit{churn}. These negative correlations are -0.14, -0.14 and -0.28. The teams with these correlation values have the mean WIP-limits of 0.73, 3.02 and 2.19. Both teams with a high mean WIP-limit and low WIP-limit have both a significant positive correlation and negative correlation.  This states that there is no pattern between WIP-limits and the correlation relationship between \textit{throughput} and \textit{churn}. 
...
The teams \textbf{two, four, six} and \textbf{seven} have a correlation of 0.67, 0.87, 0.32 and 0.77 and mean WIP-limits of 2.31, 3.21, 3.02 and 2. Teams \textbf{one, three, five, eight, nine} and \textbf{ten} have a correlation values from 0.03 to -0.18. These teams have a mean WIP-limit of 0.73, 1.78, 1.93, 0.71, 1.09 and 2.19. The correlation between \textit{WIP} and \textit{lead time} should be positive, so when WIP is decreased, then \textit{lead time} decrease as well. These data points towards the fact that WIP-limit may be too low, because four of the five teams with a WIP-limits of 2 or higher have a correlation of 0.32 or higher. 

The mean correlation between \textit{lead time} and \textit{bugs} are 0.5.  Which means that if SI produces less \textit{bugs}, the \textit{lead time} for each task will decrease, which is a good thing.  Five teams have a positive significant correlation and these teams have a WIP-limit of 0.73, 2.31, 1.91, 2.03 and 1.08. The teams \textbf{three, four} and \textbf{nine} have a correlation value of 0.44, 0.48, 0.54 and mean WIP-limits of 1.78, 3.21 and 2.19. The teams \textbf{six} and \textbf{eight} have the correlation of 0.18, -0.23 and a mean WIP-limit of 3.02 and 0.71. Based on these data, WIP-limits does not help to correlation between \textit{bugs} and \textit{lead time}, because there  is no pattern between the teams with high WIP-limit or low WIP-limit and the correlation between \textit{lead time} and \textit{bugs}.

The mean correlation between \textit{lead time} and \textit{churn} are 0.4, which means that if the effort of each tasks increase, the \textit{lead time} increase as well. Teams \textbf{one, four, five} and \textbf{eight} have a significant correlation and these teams have a WIP-limit of 0.73, 3.21, 1.93 and 0.71. The three teams with the lowest correlations are teams \textbf{six, nine} and \textbf{ten}, these three teams have a WIP-limit of 3.02, 2.19 and 1.08. The teams should have a positive correlation between  \textit{lead time} and \textit{churn}, such that the teams can work on small tasks and delivery quickly, which is one of the principle of Kanban (section ref). Based on the WIP-limits and the correlation, WIP-limits cannot be used to regulate the relationship between  \textit{lead time} and \textit{churn}.

The mean correlation between \textit{lead time} and  \textit{bugs finished quarter} and are 0.4. This relationship means that if \textit{lead time} decreases, the number if bugs closed in each quarter decreases. The relationship between these variables should be negative, so one can close more bugs and minimize \textit{lead time}. Teams \textbf{one, two, five, seven} and \textbf{ten} have a significant positive correlation and have a mean WIP-limits of 0.73, 2.31, 1.93, 2.03 and 1.08. There is one team with a negative correlation of -0.04 and that team has a mean WIP-limit of 0.71. However, the two other teams with the lowest correlations have the values 0.32 and 0.36. These two teams also have a WIP-limit of 1.78 and 3.02. These data points towards the fact that WIP-limits have no impact on the correlation between \textit{lead time} and \textit{bugs finished quarter}.
...
The paper cited by Shinkle stated; when WIP was too high, the number of bugs increased  \parencite{Shinkle}. The mean correlation between \textit{WIP} and \textit{bugs} are 0.2, which states that if WIP increases, so does bugs. Based on the paper by Shinkle, the WIP-limit is too high for SI, they should have a negative correlation. There is one team with a significant correlation and that is \textbf{team seven}  with a correlation of 0.75 as showed in Table \ref{corr:WIP:V2} and  a mean WIP-limit of  2.03 shown by Table . There is two teams with a negative correlation, these two teams are \textbf{team nine} and \textbf{team ten}. \textbf{Team nine} has the correlation of -0.05  and a mean WIP-limit of 2.19 and \textbf{team ten} has the correlation of -0.28 and a mean WIP-limit of 1.08. 
